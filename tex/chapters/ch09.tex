...
Audio and time series look like different domains, but they create the same modeling obligation: the system must respect the order in which information arrives. \emph{Prediction-time realism} means that only information truly available when the prediction is made may influence the prediction. An audio classifier cannot rely on future frames that do not yet exist at inference time, and a forecasting model cannot quietly borrow tomorrow's values while claiming to predict today. This chapter studies both domains through that one discipline. Before choosing a representation or architecture, it fixes the decision moment, the allowed context, the forecast horizon, and the latency budget. Only then do spectrograms, lag features, sequence models, and backtesting become meaningful. By the end of the chapter, the reader should be able to frame a sequential task around the decision moment and defend a time-faithful evaluation plan.

The chapter's organizing habit is a \emph{prediction-time audit}: a short written check of what information is operationally available and permitted at prediction time, what future information is forbidden, and how much delay the system can tolerate. Keep two running cases in view throughout the chapter. One is a streaming wake-word detector that must fire within a fraction of a second after hearing a target phrase. The other is a one-hour-ahead electricity-demand forecaster for a campus energy system. One consumes audio and the other consumes numeric measurements, but both are governed by the same practical question: what information was really available at the moment a decision had to be made? Those two cases also give the chapter its shape: audio makes representation and latency vivid, while forecasting makes horizon, baselines, and \emph{backtesting}---rolling evaluation forward the way the deployed system will actually be refreshed---vivid. Later sections also return to \emph{regime change}, meaning a shift in how the process behaves, and \emph{concept drift}, where the relationship the model is trying to track changes over time.

A short prediction-time audit is useful for one reason: it fixes the boundary of the claim before architecture enters the story. Once the decision moment, allowed context, forbidden future information, and latency budget are written down, the rest of the chapter can ask a more interesting question: what representation, memory choice, and evaluation protocol stay faithful to that boundary? The next section makes that shared constraint explicit before separating the domain-specific details.
...
\midrule
Early fusion & Shared low-level structure & Captures joint patterns & High-dimensional input; all modalities equally weighted \\
Late fusion & Modality independence & Interpretable per-modality; scalable & May miss complementary signals \\
...
Not all data fits naturally into grids (images), sequences (audio, text), or tables (tabular data). Some data is fundamentally relational: social networks (who follows whom), molecular structures (atoms and bonds), knowledge graphs (entities and relationships), citation networks (papers citing papers), supply chains (supplier dependencies). The structure of these relationships is a graph, and the graph topology carries information that flat or sequential representations discard.
...
Consider predicting the boiling point of a molecule from its atomic composition. Two molecules with the same atoms in different bonding arrangements (isomers) have very different boiling points. A ``bag-of-atoms'' model that counts carbon, hydrogen, and oxygen atoms cannot distinguish isomers because it ignores structure. A graph model that represents atoms as nodes and bonds as edges can learn that carbon--carbon triple bonds (alkynes) have different effects than single bonds (alkanes). In this task, graph structure is essential. The audit would ask: what information does connectivity add beyond node features?
\end{example}
...
\begin{practicalnote}
Exponential smoothing has very few parameters ($\alpha$, $\beta$ for trend, $\gamma$ for seasonality) and can be fit in seconds. It is an excellent naive baseline for any forecasting task: if a complex model cannot beat Holt-Winters, the complexity is not justified. Always include exponential smoothing in the baseline comparison.
\end{practicalnote}
...
\item \emph{Integrated (I).} Difference the series $d$ times to make it stationary (constant mean and variance). Many real-world series have trends; differencing removes them.
...
\begin{itemize}
\item Always start with exponential smoothing (Holt-Winters) and ARIMA as baselines.
\item If a neural model cannot beat the classical baseline by a meaningful margin, prefer the classical model for its transparency and lower maintenance cost.
\item For multivariate forecasting with exogenous features, consider neural approaches but still report the classical univariate baseline.
...
This state-space perspective unifies classical forecasting with the sequence models discussed in the rest of the chapter: RNNs maintain a hidden state that is updated at each time step, and Transformers can be viewed as models that attend to a context window of past states. The difference is in how the state is represented (continuous vs.\ discrete, parametric vs.\ learned) and how inference is performed (Kalman filter vs.\ backpropagation).
...
This chapter treated sequences as processes, so evaluation had to respect prediction time. The next chapter generalizes that discipline into experiment claims themselves: once the protocol stops cheating through time, we still have to ask what any score actually justifies and what rival explanation survives.

\section*{Chapter Summary}

Sequential modeling begins with the decision moment rather than with an architecture name. Once time matters, the model must respect horizon, allowed context, latency, and the difference between past information and forbidden future leakage. Classical forecasting methods---exponential smoothing, ARIMA, and their seasonal variants---remain essential baselines: they are transparent, fast, and often competitive with neural approaches on univariate series. The state-space perspective unifies classical and neural sequence models under a common framework of hidden-state inference. Audio and forecasting differ in signal type, but they share the same discipline: choose a representation that preserves the right temporal structure, evaluate chronologically, and test realism through baselines, noise, drift, and deployment constraints.
