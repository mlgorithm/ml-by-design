...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{LSTMs and GRUs} add gating mechanisms that control what information flows forward and what is discarded. Gates solve the vanishing-gradient problem for moderately long sequences.
  \item \textbf{Transformers} abandon sequential processing entirely. Attention computes direct pairwise connections between all positions, allowing any token to attend to any other without information passing through intermediate steps. This makes long-range context easy but requires encoding positional information explicitly.
\end{itemize}
...
For the demand forecaster, the simplest credible baseline is \emph{persistence}: predict that the next hour's demand equals the same hour yesterday. A slightly stronger baseline adds the weekly pattern: predict from the same hour and same day of week last week. Any model that cannot beat persistence is not learning temporal structure.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Late fusion}: Train separate models on each modality, then combine their outputs (scores, embeddings, or decisions) at the end. This assumes modalities are largely independent in their contribution to the final decision.
...
\begin{tabular}{L{3.0cm}L{2.2cm}L{2.2cm}L{2.2cm}}
\toprule
...
Cross-modal attention & Learned selective interaction & Flexible; modality-adaptive & Computationally expensive; requires rich supervision \\
\bottomrule
\end{tabular}
...
\]
where AGGREGATE pools messages from neighbors (e.g., sum, mean, max) and COMBINE updates the node's state (e.g., via a neural network). After $k$ layers, each node's representation encodes its $k$-hop neighborhood.
\end{definition}
...
Sequential modeling begins with the decision moment rather than with an architecture name. Once time matters, the model must respect horizon, allowed context, latency, and the difference between past information and forbidden future leakage. Audio and forecasting differ in signal type, but they share the same discipline: choose a representation that preserves the right temporal structure, evaluate chronologically, and test realism through baselines, noise, drift, and deployment constraints.
