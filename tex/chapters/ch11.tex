...
Chapter~\ref{ch:experiments-claims} asked what an experiment justifies. This chapter asks a harder practical question: even if the result is real, is the system reliable enough to trust under changing conditions and real consequences? Evidence is necessary, but it is not sufficient. A model can be empirically strong and still be unacceptable because it fails under shift, harms subgroups unevenly, leaks private information, or cannot fail safely. This chapter treats bias, robustness, privacy, and safety as parts of one reliability question: does the system stay within acceptable bounds for the people, conditions, and harms that actually matter in deployment? By the end of the chapter, the reader should be able to judge reliability as bounded failure across the full pipeline.
...
A \emph{predictive interval} wraps a point prediction with a range: ``the predicted value is $\hat{y}$, and we are 90\% confident the true value lies in $[\hat{y} - \delta, \hat{y} + \delta]$.'' For a reliable system, the interval must be well-calibrated: 90\% intervals should contain the true value 90\% of the time.
