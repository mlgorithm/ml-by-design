...
Consider a hiring classifier evaluated on two groups, A and B. Group A has a base rate of 40\% qualified candidates; Group B has 20\%. In a simple toy case, suppose the model has the same true positive and false positive rates in both groups, with TPR $= 0.80$ and FPR $= 0$. It then satisfies \emph{equalized odds} but still violates \emph{demographic parity}: it accepts 32\% of Group A ($0.80 \times 0.40$) but only 16\% of Group B ($0.80 \times 0.20$). No nontrivial classifier can generally satisfy both criteria when base rates differ, except in degenerate cases such as predicting all negative or all positive. This tension forces an explicit design choice about which fairness property the system should prioritize.
\end{example}
