...
This chapter turns fairness, robustness, privacy, and safety into one bounded-failure review. After reading it, students should ask for whom the system fails, under what shifts, and which hazards block launch instead of treating reliability as a moral appendix. The reliability review card extends the experiment claim sheet into deployment-facing risk judgment.
\end{whychaptermatters}

The previous chapter argued that experimental results only matter when claims match evidence. Reliability extends that discipline into higher-stakes questions and turns it into a deployment constraint. It asks not only whether a model works on average, but for whom, under what shifts, with what privacy costs, and with what safety margins when failure matters. The chapter therefore moves in a specific order: identify who is unevenly served, test how those failures change under shift, trace what information might leak, bound the failures that are unacceptable, and then decide what monitoring and fallback make deployment defensible. The chapter culminates in a reliability review card, so the early slice dashboards and harm models should be read as preparation for one launch decision rather than as a detached ethics checklist.

\begin{problemsolvinglens}
This chapter asks a broader question than average performance: for whom does the system work, under what changes does it fail, and which risks are serious enough to block deployment?
...
\caption{Reliability review is a matrix of checks, not one aggregate score. Each row names a failure dimension, each column names a pipeline stage, and each cell asks what evidence and fallback are needed there.}
...
\begin{tabular}{L{2.75cm}L{6.55cm}}
\toprule
Review field & Early filled example for the course-support assistant \\
\midrule
Critical slice & Multilingual or distressed student messages that may require escalation even when routine policy questions are safe \\
Shift to test & New semester wording, revised policy pages, or changed course-tool terminology \\
Privacy exposure & Student identifiers, health disclosures, accommodation records, and advisor notes \\
Severe hazard & The assistant answers a high-risk message directly instead of escalating with evidence attached \\
Fallback / no-launch rule & Escalate by default on high-risk categories; delay launch if severe-slice failures remain unresolved \\
\bottomrule
\end{tabular}
\caption{A compact filled review card preview makes the chapter's destination concrete before the full card appears later.}
\label{tab:early-reliability-review-preview}
\end{table}
...
Reliability review starts with a harm model, not with a metric menu. Until the chapter has named who can be harmed, how, and under what shift or exposure, no single fairness, robustness, privacy, or safety number is informative enough.
\end{minimumtakeaway}
...
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{L{1.08cm}L{1.00cm}L{1.00cm}L{1.00cm}L{1.00cm}L{1.48cm}L{1.56cm}}
\toprule
Group & TP & FN & FP & TN & TPR / FPR view & Positive rate \\
\midrule
A & 48 & 12 & 4 & 36 & $\mathrm{TPR}=0.80$, $\mathrm{FPR}=0.10$ & $52/100=0.52$ \\
B & 16 & 4 & 8 & 72 & $\mathrm{TPR}=0.80$, $\mathrm{FPR}=0.10$ & $24/100=0.24$ \\
\bottomrule
\end{tabular}
\caption{Two fairness views can disagree. Here equalized-odds style rates match across groups, but positive-prediction rates do not because the base rates differ.}
\label{tab:fairness-metrics-disagree}
\end{table}

\begin{practicalnote}
Table~\ref{tab:fairness-metrics-disagree} is exactly why the chapter says metrics are tools, not magic. One reviewer may say the system looks balanced under equalized odds. Another may object that service exposure is still very unequal. The right next step is to ask which harm model matters for the deployment, not to hunt for one metric that ends the argument.
\end{practicalnote}
...
\begin{example}[A Small Privacy Failure Before Any Fancy Attack]
Suppose a course-support team exports $200$ real student messages to a shared spreadsheet so interns can compare prompts. Several messages mention disability accommodations, health events, and visa concerns. No model attack is needed: the sensitive fact is already exposed through the debugging workflow. The first privacy lesson is therefore concrete rather than exotic. Before choosing tools, ask who can see what sensitive field through which ordinary data path.
\end{example}
...
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{L{2.42cm}L{6.04cm}}
\toprule
Safety-case field & Filled support-assistant example \\
\midrule
Hazard & high-risk student message is answered directly instead of escalated \\
Trigger condition & mixed routine wording plus distress, visa, disability, or disciplinary cues \\
Mitigation & high-risk intent detection, grounded retrieval, conservative answer thresholds, and explicit refusal rules \\
Fallback & immediate routing to trained human staff with the retrieved evidence attached \\
Monitoring signal & rise in escalation misses, unsupported direct answers, or overrides on audited high-risk slices \\
\bottomrule
\end{tabular}
\caption{A safety case becomes more usable for undergraduates when one real service is written out field by field rather than only described in prose.}
\label{tab:filled-safety-case}
\end{table}
...
\begin{example}[When the Right Review Ends in ``Do Not Deploy'' ]
Suppose a support assistant shows good routine-message performance, but the multilingual visa slice still has weak routing recall, raw support logs retain sensitive health details for debugging, and the advising office cannot staff the required fallback queue outside business hours. None of those findings is a cosmetic concern. Together they mean the current system cannot bound the harms it would create in real use. The technically correct conclusion is to delay launch, narrow the use case, or redesign the pipeline rather than to deploy and hope monitoring will catch the failures later.
\end{example}

\begin{decisionbox}
If a severe slice failure remains, a privacy exposure has no credible mitigation, or the fallback path cannot actually be staffed in the real workflow, the system is not merely ``imperfect.'' It is not yet deployable on honest reliability grounds.
\end{decisionbox}
...
\midrule
Core focus & The data and deployment conditions that create risk, the evidence that bounds failure, and the review or fallback that keeps the system acceptable \\
...
These mistakes are surface forms of the chapter's main enemy: mistaking average success for bounded, acceptable failure. Read them as one reliability checklist rather than five separate mini-topics.
...
The reliability review card from this chapter becomes the starting document for Chapter 13's system readiness brief: once bounded failure is explicit, the next question is whether the whole service is ready to operate.
...
\item Group A has confusion counts $(TP=45, FN=5, FP=30, TN=120)$ and Group B has confusion counts $(TP=18, FN=2, FP=4, TN=56)$. Compute the true positive rate and false positive rate for both groups. Which group faces the higher false-positive burden, and what reliability concern does that raise in a screening or review system?
\item Using Table~\ref{tab:fairness-metrics-disagree}, explain why equalized-odds style parity can hold while positive-prediction rates still differ sharply. Then say which harm model would make that difference matter most.
