...
Student 1's weighted score is \(0.65\), meaning the model predicts they are moderate risk: reasonable attendance partially offsets one late submission. Student 2's score of \(7.10\) signals high risk because four late submissions dominate. Student 3's score of \(-1.20\) indicates low risk: solid attendance with no late submissions.
...
\emph{Missingness in tabular data is often not random.} Imputing a missing value with a mean or zero assumes the value was random noise. But if the value is missing because the student dropped before week eight, or because a sensor was offline after a power failure, the imputation will inject a different kind of noise. Later chapters on reliability and deployment return to this point. For now, the lesson is that ``missing'' is a value in the representation, not an error to be erased before modeling.
...
\midrule
Dot products and weighted sums & Chapters 2, 5 (linear models, neural networks) \\
Norms and distance & Chapters 3, 7 (nearest neighbors, regularization) \\
Cosine similarity & Chapters 6, 8 (embeddings, retrieval) \\
Matrix--vector products & Chapter 5 (forward pass, backpropagation) \\
Linear separability & Chapters 3, 5 (baselines, learned features) \\
\bottomrule
\end{tabular}
\end{table}
