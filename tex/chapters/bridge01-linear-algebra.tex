...
\begin{mathlens}
Given a centered data matrix $\mathbf{X} \in \mathbb{R}^{n \times d}$ (rows are examples, columns are features), the SVD yields $\mathbf{X} = \mathbf{U} \boldsymbol{\Sigma} \mathbf{V}^\top$. The top $k$ principal components are the first $k$ columns of $\mathbf{V}$. Projecting the data onto these components gives reduced coordinates $\mathbf{Z}_k = \mathbf{X} \mathbf{V}_k$; reconstructing from those coordinates gives the rank-$k$ approximation $\widetilde{\mathbf{X}}_k = \mathbf{Z}_k \mathbf{V}_k^\top$. Explained variance is tied to the squared singular values: if the first 5 principal components explain 95\% of the total centered variance, the data can often be well approximated in 5 dimensions even if it has many raw features.
\end{mathlens}
