...
\begin{codeexample}[caption={From scratch: matrix--matrix product with explicit shape assertions}]
def matmul(A, B):
    n, k = len(A), len(A[0])
    k2, m = len(B), len(B[0])
    if k != k2:
...
Three loops, no library magic. The outer two loops walk every output cell; the inner loop is the dot product of row $i$ of $A$ with column $j$ of $B$. The shape assertion at the start is the most common bug-catcher in matrix code: most ``broken model'' issues in a layered network are silent shape mismatches that this assertion would have raised immediately. Numpy's \verb|A @ B| is the same operation, vectorized and parallelized; the loops above are just the literal definition.
...
\textbf{Three matrix-calculus facts.} For a scalar-valued function of a vector \(w \in \mathbb{R}^d\), let \(\nabla_w f\) denote its gradient (the column vector of partial derivatives).
...
\emph{(3) Squared error.} If \(f(w) = \|y - X w\|_2^2\), then \(\nabla_w f = -2 X^\top (y - X w)\). Setting this to zero gives the \emph{normal equations} \(X^\top X w = X^\top y\)---the closed-form least-squares solution.
...
\item \exercisetag{Implementation}{Intermediate}Implement a from-scratch matrix--vector multiply \verb|matvec(A, x)| using nested Python loops, with a shape assertion at the start. Test it against the worked Example in this chapter so that \verb|matvec(X, w)| reproduces the predictions table. Then time your loop version against \verb|numpy| on a $100 \times 100$ random $A$ and report the ratio. The point is not to make Python fast; it is to feel where the work actually happens before letting a library hide it.
\end{exercises}
