...
\draw[thick, dashed] (0.8,-0.05) -- (0.8,1.8);
...
\begin{mathlens}
Given a centered data matrix $\mathbf{X} \in \mathbb{R}^{n \times d}$ (rows are examples, columns are features), the SVD yields $\mathbf{X} = \mathbf{U} \boldsymbol{\Sigma} \mathbf{V}^\top$. The top $k$ principal components are the first $k$ columns of $\mathbf{V}$. Projecting the data onto these components gives $\mathbf{X}_k = \mathbf{X} \mathbf{V}_k$, which captures the $k$ directions of greatest variance while discarding the rest. If the first 5 singular values capture 95\% of the total centered variance, the data effectively lives in 5 dimensions regardless of how many raw features exist.
\end{mathlens}
...
\midrule
Dot products and weighted sums & Chapters~\ref{ch:prediction-loss-decision},~\ref{ch:optimization-neural-networks} (linear models, neural networks) \\
