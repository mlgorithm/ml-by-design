...
\begin{prereqbridge}
Use this chapter as just-in-time review: read it now if linear-algebra notation feels rusty, or return when later chapters start leaning on vectors, matrices, and geometry. This chapter assumes comfort with high-school algebra: variables, substitution, and evaluating expressions. Calculus is not required. Familiarity with Python lists or arrays is helpful but not required. Students who have already taken a linear algebra course should still skim this chapter because the AI-specific vocabulary (representation, coordinate, weighted evidence) will recur throughout the book.
\end{prereqbridge}
...
This bridge chapter is therefore not a miniature mathematics course. It is a reading course in the mathematical objects that later chapters will use constantly. If a student can look at a vector or matrix and ask, ``what real thing does each row, column, or coordinate mean here?'' then the mathematics is already doing its job.
...
Keep the chapter artifact in view from the start. As later sections introduce shapes, weighted scores, geometry, and projection, the intended payoff is a short \emph{representation card}: what one example is, what the coordinates mean, what geometry matters, and what a linear score would mean in plain language.
...
Those encodings are not interchangeable. Treating unknown as zero can make a model learn that absence of a record is the same as poor performance.

Three practical lessons follow.

\emph{Sparse representations support fast inner products but can break distance-based methods.} If most of two vectors' coordinates are zero, their Euclidean distance is dominated by the dimensions where both are nonzero---or by the few nonzero dimensions of one but not the other. Cosine similarity can handle this better in some cases, but neither operation treats ``both missing this word'' and ``both present on this word'' the same way.
...
\emph{Missingness in tabular data is often not random.} Imputing a missing value with a mean or zero often treats the missing entry as if it were typical, absent, or otherwise uninformative under the chosen coding. But if the value is missing because the student dropped before week eight, or because a sensor was offline after a power failure, the imputation can inject a different kind of distortion. Later chapters on reliability and deployment return to this point. For now, the lesson is that ``missing'' is a value in the representation, not an error to be erased before modeling.
...
Later chapters use these ideas for regularization. The reason is simple. A model with very large coefficients can become brittle, overly reactive, or too willing to fit accidental quirks in the training data. Penalizing large coefficient size is one way of expressing a preference for simpler behavior unless the data strongly demands otherwise.
...
A linear score $w^\top x + b$ divides the input space by the boundary $w^\top x + b = 0$. One side has positive score, the other has negative score, and points with score exactly zero lie on the boundary itself. In two dimensions, that boundary is a line. In three dimensions, it is a plane. In $d$ dimensions, it is a hyperplane.
...
\textbf{Eigenvectors and eigenvalues.} For a square matrix $\mathbf{A}$, an eigenvector $\mathbf{v}$ satisfies $\mathbf{A}\mathbf{v} = \lambda \mathbf{v}$: the matrix stretches $\mathbf{v}$ by a factor $\lambda$ (the eigenvalue) without changing its direction. For a covariance matrix, eigenvectors are principal axes; ordered by eigenvalue, larger eigenvalues correspond to more variance along that direction.

\textbf{Singular Value Decomposition.} Any matrix $\mathbf{X}$ (not just square ones) can be decomposed as $\mathbf{X} = \mathbf{U} \boldsymbol{\Sigma} \mathbf{V}^\top$, where $\mathbf{U}$ and $\mathbf{V}$ are orthogonal matrices and $\boldsymbol{\Sigma}$ is diagonal with non-negative entries (the singular values). The columns of $\mathbf{V}$ give orthogonal directions ordered by how much squared norm of the matrix they capture. When $\mathbf{X}$ is centered first, those directions become the principal-component directions of greatest variance. PCA is therefore computed from the SVD of a centered data matrix: the top $k$ columns of $\mathbf{V}$ are the first $k$ principal components.
...
Task: determine whether an incoming email is spam. Each row in the dataset is one email. The coordinates are chosen features: word frequencies from a bag-of-words model, sender reputation score, number of links in the message, and header flags (all-caps subject, suspicious domains, etc.).

With a vocabulary of 50 words and 5 numeric features (reputation, link count, and three binary flags), the representation lives in $\mathbb{R}^{55}$. The first 50 coordinates are term frequencies. Coordinates 51–55 are the numeric metadata.

A critical note on representation choice: the bag-of-words representation discards word order entirely. The sequence ``Nigerian prince has offer'' and ``offer has prince Nigerian'' both map to the same vector because only counts matter, not position. This is already a modeling decision. We are saying that detecting spam does not require understanding syntax or sequence, only statistics of presence and absence. That choice is often reasonable for spam (many phishing emails do repeat the same words in any order), but it would fail for sentiment analysis (``not good'' means something very different from ``good not''). Filling out a representation card before choosing a model family makes such assumptions visible.
\end{example}

\begin{example}[Representation Card Row for an Image Patch]
For a simple image task, one example might be a \(32 \times 32\) image patch. The raw coordinates are pixel intensities, or three color channels for each pixel. What is lost depends on preprocessing: resizing may blur small objects, cropping may remove context, and converting to grayscale discards color. The geometry matters locally: nearby pixels and small translations are meaningful, while arbitrary coordinate swaps are not. A linear score on raw pixels would mostly detect fixed templates; a convolutional model is introduced later because it can ask repeated local questions across the image.
\end{example}
...
The SVD preview above gives just enough eigendecomposition intuition to prepare for PCA, embeddings, and matrix factorization in later chapters. The full machinery appears when those topics demand it.
...
Norms and distance & Chapters~\ref{ch:linear-models},~\ref{ch:representation-foundation-models} (nearest neighbors, regularization) \\
