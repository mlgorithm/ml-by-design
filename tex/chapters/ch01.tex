...
The chapter moves in a deliberate order. First, we sharpen the distinction between prediction and decision. Then we introduce the Chapter 1 framing protocol through the Five Commitments, repair vague goals through proxy audit, treat data as evidence rather than truth, preview why evaluation must use unseen evidence, and finally turn the whole discussion into one framing memo.
...
This table is intentionally incomplete. Its job is only to make the chapter's destination visible early. The fuller framing memo later in the chapter adds excluded inputs, evaluation preview, and cleaner justification for the task.
...
\draw[rounded corners=8pt, fill=c1aiBg, draw=c1aiLine!45, line width=1pt]
    (0,0) rectangle (11.0,5.0);
%% ── Middle ML zone ───────────────────────────────────────────────────────
\draw[rounded corners=6pt, fill=c1mlBg, draw=c1aiLine!65, line width=1pt]
    (0.8,0.7) rectangle (8.6,4.1);
%% ── Inner DL zone ────────────────────────────────────────────────────────
\draw[rounded corners=4pt, fill=c1dlBg, draw=c1aiLine, line width=1.2pt]
    (1.6,1.4) rectangle (6.2,3.2);
%% ── Zone labels ──────────────────────────────────────────────────────────
\node[anchor=north west, font=\small\bfseries, text=c1aiLine!55!black]
    at (0.25,4.78) {Artificial Intelligence};
\node[anchor=north west, font=\small\bfseries, text=c1aiLine!80!black]
    at (1.05,3.88) {Machine Learning};
\node[anchor=north west, font=\small\bfseries, text=white]
    at (1.85,3.0) {Deep Learning};
%% ── AI-only examples ─────────────────────────────────────────────────────
\node[align=center, text width=2.3cm, font=\footnotesize, text=c1aiLine!55!black]
    at (9.7,2.9) {planning\\[3pt]search\\[3pt]rule systems};
%% ── ML-only examples ─────────────────────────────────────────────────────
\node[align=center, text width=2.0cm, font=\footnotesize, text=c1aiLine!80!black]
    at (7.3,1.7) {decision trees\\[3pt]linear models\\[3pt]clustering};
%% ── DL examples ──────────────────────────────────────────────────────────
\node[align=center, text width=2.4cm, font=\footnotesize, text=white]
    at (3.9,2.1) {vision nets\\[3pt]language nets\\[3pt]multilayer nets};
\end{tikzpicture}%
}
...
One reason beginner projects drift so quickly is that four different objects get collapsed into one vague sentence. A team says it wants to ``improve retention'' or ``support students better,'' then jumps directly to labels and metrics as if they were the same thing as the institutional goal. They are not.

For beginners, three words must stay separate: the \emph{target} is what the model predicts, the \emph{objective} is what training minimizes, and the \emph{metric} is the number reported after development. In student support, the target might be early disengagement risk, the objective might be log loss on the labeled examples, and the metric might be recall under a fixed review budget. A project can optimize one quantity, train on another, and be judged by a third, so the names are not interchangeable.
...
Many beginner mistakes come from starting with a favorite model rather than with the structure of the problem. A more reliable approach is to identify five commitments before training begins.
...
Objective & What behavior is rewarded in training? & Surface students for support early enough to intervene & Separate spam from wanted mail at acceptable cost & Predict the local count signal well enough that the final count is accurate \\
...
Chapters 2 and 3 are the real judgment chapters, so evaluation appears here only as a preview. The essential preview is simple: a learning problem is not defined only by its inputs and targets. It is also defined by what counts as unseen evidence.
...
\begin{tikzpicture}[
  ...
  visbox/.style={draw=blue!40, rounded corners=4pt, align=center,
                 minimum height=1.1cm, minimum width=3.1cm,
...
%% ── Top row: data splits ─────────────────────────────────────────────────
\node[visbox] (train) at (0,   2.1) {Visible training set\\images + density maps};
\node[hidbox] (valid) at (4.5, 2.1) {Hidden evaluation\\counts not shown};
\node[hidbox] (test)  at (9.0, 2.1) {Final hidden set\\counts not shown};
%% ── Model node ───────────────────────────────────────────────────────────
\node[modbox] (model) at (0,   0.3) {Model development\\on visible data only};
%% ── Divider between visible and hidden regions ───────────────────────────
\draw[dashed, gray!45, line width=0.8pt] (2.05,2.85) -- (2.05,-0.4);
\node[font=\scriptsize\bfseries, blue!45!black]  at (0.9, 2.72) {VISIBLE};
\node[font=\scriptsize\bfseries, gray!55!black]  at (6.5, 2.72) {HIDDEN};
%% ── Arrows ───────────────────────────────────────────────────────────────
\draw[arrow] (train.south) -- (model.north);
\draw[arrow] (model.east)  to[out=0,  in=230] (valid.south west);
\draw[arrow] (model.east)  to[out=0,  in=250] (test.south);
\end{tikzpicture}%
}
...
This is the test for Chapter 1. If the reader can take a messy idea and produce a memo like Table~\ref{tab:framing-memo}, the chapter has done its real job. Everything earlier in the chapter should cash out into this document.
...
The chapter's main deliverable is now on the page, so only brief orientation remains. Machine learning is often divided into supervised, unsupervised, self-supervised, and reinforcement learning. The most useful way to think about those categories here is not as separate museum rooms, but as different \emph{feedback regimes}.

In supervised learning, the feedback is explicit labels. In unsupervised learning, there are no explicit targets and the system must discover structure on its own. In self-supervised learning, the system creates training targets from the data itself. In reinforcement learning, the feedback arrives through rewards and consequences of actions over time.
...
Unsupervised\,/\,self-supervised & Structure in the data itself & When labels are scarce or the goal is to learn reusable representations \\
...
\bottomrule
\end{tabular}
\caption{The three main feedback regimes differ in where the learning signal comes from and in the kind of task they suit best.}
\label{tab:feedback-regimes}
\end{table}

\begin{practicalnote}
This book's center of gravity is supervised and self-supervised machine learning. Reinforcement learning appears here only for orientation. Students who want to go deeper should know that RL is the framework behind game-playing agents, robotic control, and the preference-alignment stage of modern language model training --- topics briefly introduced in later chapters.
\end{practicalnote}

\begin{decisionbox}
Use supervised learning when labeled examples are available and the task is clearly defined. Consider self-supervised approaches when labels are scarce but raw data is abundant. Reinforcement learning applies when the right action can only be discovered through interaction with an environment and cannot be defined in advance as a labeled target.
\end{decisionbox}
...
This chapter framed the problem: what is the task, what is the proxy, and what counts as a defensible learning problem? The next two chapters turn that framing into judged evidence. Chapter 2 asks what predictive behavior counts as good for the decision in front of us. Chapter 3 then asks how to protect that judgment from leakage, weak baselines, and accidental misuse of held-out evidence.
