...
\begin{learninggoals}
\item Distinguish only briefly between the broader field of AI and the narrower practice of machine learning so the scope of the book is clear.
\item Explain why a model score is not yet a decision or an intervention.
\item Use the Chapter 1 framing protocol, organized through the Five Commitments, to turn a vague AI idea into a defensible learning problem.
\item Audit proxy targets, data, and acceptable inputs before choosing a model.
\item Treat datasets as evidence rather than truth, and explain why later evaluation must use unseen evidence.
\item Produce a framing memo before any model training begins.
\end{learninggoals}

\begin{whychaptermatters}
This chapter turns a vague AI request into a defensible learning problem. After reading it, students should be able to name the user, action, proxy, acceptable inputs, and review path before they talk about models. The framing memo becomes the starting document for the metric-and-action worksheet and evaluation plan in the next two chapters.
\end{whychaptermatters}

Many AI projects begin with a sentence that sounds ambitious and helpful but is too vague to support technical work. ``Use AI for student success.'' ``Detect bad content.'' ``Find fake news.'' None of these is yet a defensible learning problem. They are intentions, not specifications. If a team starts coding from that level of vagueness, it often builds a system whose output is mathematically polished and practically confused.

This chapter is the first workshop of the book. Its job is not to survey all of AI. Its job is to teach how a messy request becomes a task, how a score differs from an action, how collected data stands in for the world, and how to write down enough of the problem that model choice becomes meaningful rather than premature. By the end, the reader should be able to produce a framing memo before touching a model.

The chapter moves in a deliberate order. First, we sharpen the distinction between prediction and decision. Then we introduce the Chapter 1 framing protocol through the Five Commitments, repair vague goals through proxy audit, treat data as evidence rather than truth, preview why evaluation must use unseen evidence, and finally turn the whole discussion into one framing memo. Read the chapter as a sequence of justifications for that memo: each major section makes one later row of the artifact more precise.

\section{Problem-Solving Technique: Decision-First Framing}

Before the chapter opens up the full dilemma, it helps to state the main habit in compact form. The core move is simple: name the decision and its constraints before you start thinking about models.
...
\begin{practicalnote}
A simple workflow is enough to start: write the decision, name the user, choose a proxy target, list acceptable and excluded inputs, and then draft a memo that makes the action and review path explicit. If that sequence feels fuzzy, the problem is still not ready for model choice.
\end{practicalnote}

\begin{warningbox}
\textbf{What goes wrong if you skip this chapter?} Teams that start with a model instead of a framing memo often optimize a target they cannot defend, rely on inputs that would not exist at prediction time, and report a strong score for an action nobody has specified. The result can look technically polished while still being operationally unusable or ethically misaligned.
\end{warningbox}

\section{Opening Dilemma: A Messy Real Problem}

Suppose a university asks: \emph{Can we identify students who may need help early?} That sounds like a sensible use of AI. It is also badly underframed, meaning that the goal has not yet been translated into a usable technical specification.
...
That sentence is worth treating as the chapter's first checkpoint. If the problem statement is still vague, later mathematical sophistication will mostly hide confusion rather than solve it.
...
Proxy target & Risk of missing early major deadlines rather than a vague idea such as ``weak student'' \\
...
\begin{practicalnote}
The same course-support mini-case carries the opening three chapters. Here it becomes a framing memo. In Chapter 2 it becomes a metric-and-action worksheet with thresholds, ranking, and calibration choices. In Chapter 3 it becomes an evaluation plan with protected splits, leakage controls, and baselines.
\end{practicalnote}
...
That notation is enough for now. Chapter 1 is not about manipulating formulas. It is about deciding what the symbols should refer to in the first place.
...
Proxy target & What narrower learning task stands in for that goal? & Predict risk of missing the first two major deadlines early enough to intervene & Choosing a proxy that is easy to log but poorly aligned with the real need \\
Label & How will the proxy be turned into observed targets? & Mark a case positive if the student misses one of the first two major deadlines or omits core work by the cutoff & Treating administratively produced labels as ground truth rather than as measurements \\
Metric & What number will summarize success? & Recall among students who later disengaged, measured under a fixed advising-review budget & Optim\-izing a convenient number that ignores the decision constraint \\
\bottomrule
\end{tabular}
...
The difference becomes clearer in one concrete student-support story. The institutional goal is not ``predict failure'' in the abstract. The goal is to help students receive timely human support. The proxy target might therefore be early disengagement risk rather than final-course failure, because waiting for final outcomes would make the intervention too late. The label then depends on a measurable rule, such as missed core work by a documented cutoff. The reported metric may finally be recall at a fixed review budget because the advising team can inspect only a limited number of cases each week.
...
By this point, the chapter has already separated goal, proxy target, label rule, and metric. The next step is to turn that framing into a checklist a team can actually use before training. The Five Commitments are that checklist.

\begin{enumerate}[leftmargin=*,label=\arabic*.]
\item \textbf{Task.} What exactly is the system supposed to predict, estimate, or rank?
...
These five commitments are Chapter 1's working framing checklist inside the book's broader seven-question framework. They explain how a vague request becomes a learnable task before baseline, reliability, and systems questions are added later.
...
The Five Commitments are a smaller working version of that larger framework. Task mainly answers Questions 1 and 2. Data answers Question 3. Representation answers Question 4. Evaluation previews Question 6. Questions 5 and 7 are delayed because a framing chapter should first make the problem clear before it asks what baseline is credible or what deployment system will be responsible for the output. Objective is added here because once a task is framed, training still needs a concrete signal about what the model should learn to do.
...
Students often meet these frameworks one after another and wonder whether they are competing maps. They are not. Table~\ref{tab:framework-map} shows how they nest.

\begin{table}[t]
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{L{2.58cm}L{3.02cm}L{4.23cm}}
\toprule
Framework & Main job in Chapter 1 & How it connects to the rest of the opening sequence \\
\midrule
Objective hierarchy & Separates goal, proxy target, label rule, and reporting metric so the levels of the problem do not blur together & The Chapter 1 memo records those distinctions in concrete language before Chapter 2 turns the metric and action policy into a worksheet \\
Five Commitments & Turns framing into an operational checklist: task, data, representation, objective, and evaluation & This is the chapter's working protocol for deciding whether the project is ready for model development at all \\
Seven-question framework & Gives the larger book-level spine: decision, proxy, evidence, representation, baseline, claim, and system & Chapter 1 settles the early questions first; later chapters add the baseline, experiment, reliability, and systems layers \\
Chapter artifact & Produces one written deliverable the student can actually carry forward & The framing memo becomes the starting document that Chapters 2 and 3 extend rather than replace \\
\bottomrule
\end{tabular}
\caption{The framing tools are nested rather than competing. The hierarchy separates levels of meaning, the Five Commitments make the project checkable, the seven questions forecast the whole book, and the artifact writes down the current answer.}
\label{tab:framework-map}
\end{table}

\begin{practicalnote}
First-pass rule: use the objective hierarchy to keep meanings separate, use the Five Commitments to check readiness, and use the framing memo to record the current answer. The seven-question framework is the larger map of the whole book, not one more list that must be mastered all at once.
\end{practicalnote}
...
\begin{tabular}{@{}>{\raggedright\arraybackslash}p{1.85cm}>{\raggedright\arraybackslash}p{1.50cm}>{\raggedright\arraybackslash}p{1.80cm}>{\raggedright\arraybackslash}p{1.76cm}>{\raggedright\arraybackslash}p{3.09cm}@{}}
\toprule
Commitment & Guiding question & Student support & Spam filtering & Counting task \\
\midrule
Task & What must be predicted? & Risk of missing early deadlines & Probability an email is spam & Animal count from an aerial image \\
Data & What evidence stands in for the world? & Assessments, attendance, and platform activity & Labeled email text and message details & Images paired with local count targets \\
Input form & What form does the model actually see? & Early-course signals in a student table & Words, links, sender cues, and email metadata & Pixels, local patches, and count cues \\
Objective & What behavior is rewarded in training? & Minimize a training loss that raises scores on students needing timely support & Minimize a training loss that separates spam from wanted mail & Minimize error between predicted and observed local count targets \\
...
\begin{thinkingtool}
\textbf{Tempting mistake $\rightarrow$ corrected reasoning.} Tempting mistake: ``The project sounds helpful, so we can start modeling.'' Corrected reasoning: ``Until the user, downstream action, proxy target, and most costly mistake are explicit, we still have an intention rather than a defensible learning problem.''
\end{thinkingtool}
...
Versioning is the practical habit that makes data-centric work scientific rather than anecdotal. If a team removes duplicates, relabels borderline cases, adds examples from an underrepresented setting, or changes preprocessing, then the dataset has changed. The result should be treated as a new version with its own documentation. Chapter 11 returns to this experimental discipline, but the habit starts here, while the problem is still being framed.
...
\midrule
Seven-question focus & The memo makes the decision, proxy target, available evidence, and representation explicit. It also previews what later evidence would justify the claim and how the prediction would enter downstream action and review. \\
...
Training objective & Minimize a supervised risk loss that helps surface likely disengagement cases early enough for human support \\
...
\begin{thinkingtool}
\textbf{Framing memo.} Before training, write down the decision, proxy target, inputs, excluded inputs, costliest mistake, evaluation preview, downstream action, and review path. If those fields remain vague, the project is still underframed.
\end{thinkingtool}

This is the test for Chapter 1. If the reader can take a messy idea and produce a memo like Table~\ref{tab:framing-memo}, the chapter has done its real job. Everything earlier in the chapter should cash out into this document. Keep the memo. In Chapter 2, the evaluation preview inside it will become explicit metrics, action rules, and probability checks, and in Chapter 3 it will become part of a defended evaluation plan.
...
Suppose the advising team trains an early model and sees three surprises:
\begin{itemize}[leftmargin=*]
\item many flagged students recover quickly without any support action, so the proxy target may be too noisy,
\item one subgroup is systematically under-flagged because early platform activity is a weak signal for their course format,
\item the current threshold would send far more students to review than the advising team can actually contact.
\end{itemize}
...
\midrule
Proxy target & missed deadlines were too coarse and mixed routine recovery with serious disengagement & refine the proxy toward repeated non-submission or combine it with review notes \\
...
The chapter's main deliverable is the framing memo. One short orientation is still useful because later chapters refer to supervised, unsupervised, self-supervised, and reinforcement learning. The important point here is not to memorize a taxonomy. It is to recognize that these settings differ in the kind of feedback they provide, and that the available feedback constrains what the system can honestly learn.

These are not stages on a ladder. They are different regimes for learning from data or interaction.

\begin{table}[t]
\centering
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{L{2.65cm}L{3.7cm}L{3.55cm}}
\toprule
Feedback regime & Source of feedback & Typical use \\
\midrule
Supervised learning & Explicit labels provided by people or other processes & Predict student-risk labels from past cases with observed outcomes \\
Unsupervised learning & Structure in the data itself & Group similar behavior patterns or find lower-dimensional structure \\
Self-supervised learning & Targets created from raw data itself & Learn reusable language or image representations before a downstream task is fixed \\
Reinforcement learning & Rewards from actions and their later consequences & Learn behavior when good action must be discovered through interaction over time \\
\bottomrule
\end{tabular}
\caption{Learning settings differ mainly in where the feedback comes from, not in where they sit on a spectrum.}
\label{tab:feedback-regimes}
\end{table}

\begin{practicalnote}
This book's center of gravity is supervised and self-supervised machine learning. Reinforcement learning appears here only for orientation. The useful first-course contrast is simple: RL does not begin with a fixed dataset of correct answers. It learns from the consequences of actions.
\end{practicalnote}

\begin{decisionbox}
Before asking which learning setting to use, ask the earlier framing question: what feedback is actually available, and does it match the decision problem honestly enough to support a defensible claim?
\end{decisionbox}

\section{Common Mistakes in First Machine Learning Projects}

Several mistakes appear so often that they are worth naming early.

\begin{mistakebox}
\item \textbf{Starting with the model instead of the problem.} Begin with the task, the data, the consequences of mistakes, and the evaluation plan; choose the model only after those are clear.
\item \textbf{Treating labels as perfect truth.} Labels are noisy measurements, not ground truth.
\item \textbf{Ignoring the decision context.} A prediction matters only inside a decision process, so identify who acts and what happens next.
\item \textbf{Confusing correlation with explanation.} Predictive features may be proxies or shortcuts, and correlation alone is not causation.
\end{mistakebox}

\section{Looking Ahead}

This chapter framed the problem: what is the task, what is the proxy, and what counts as a defensible learning problem? The next two chapters turn that memo into something we can test. Chapter 2 asks what good predictive behavior looks like for the decision in front of us. Chapter 3 then asks how to protect that evaluation from leakage, weak baselines, and accidental misuse of held-out data. In other words, Chapter 1 states the problem clearly; Chapters 2 and 3 check whether later evidence really supports that statement.

This chapter's artifact becomes the starting document for the next chapter's artifact chain: the framing memo becomes the metric-and-action worksheet in Chapter 2, and that worksheet becomes the evaluation plan in Chapter 3.

\begin{decisionbox}
Ready to model only if you can name the user, the action, the proxy target, the available-at-prediction-time inputs, the unacceptable inputs, the costliest mistake, the held-out evidence, and the review path. If any piece is still vague, stop and reframe the problem before training. This framing discipline will resurface throughout the book, from evaluation design (Chapters 2 and 3) to reliability assessment (Chapter 12) to system deployment (Chapter 13).
\end{decisionbox}

\begin{chaptersummary}
\item The first job in AI is not model choice. It is turning a vague request into a defensible learning problem.
\item A prediction is not the same as a decision. Scores become meaningful only inside a downstream policy or intervention.
\item The Five Commitments are Chapter 1's framing protocol inside the broader seven-question framework: task, data, representation, objective, and evaluation.
\item Proxy targets must be audited because real-world goals and measurable labels are rarely identical.
\item Datasets are evidence, not truth; they are shaped by sampling, annotation, timing, and interface decisions.
\item Data-centric ML asks whether coverage, labeling, documentation, and versioning should be improved before reaching for a more complex model.
\item Supervised, unsupervised, self-supervised, and reinforcement learning are best understood as different feedback regimes.
\item Evaluation enters Chapter 1 only as a preview: a claim must eventually survive hidden or held-out evidence.
\item The main deliverable of the chapter is a framing memo written before any model training begins.
\end{chaptersummary}
...
\item A student-support team proposes using ZIP code, prior disciplinary history, current course activity, and recent assignment submissions as inputs to an outreach model. For each input, decide whether it should be accepted, rejected, or flagged for further review before training. Justify your answer in terms of prediction-time availability, proxy risk, and downstream action.
