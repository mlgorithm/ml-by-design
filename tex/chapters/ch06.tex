...
A model can succeed on one task in two very different ways. It can learn a narrow solution tied closely to that task, or it can learn a representation that remains useful when the task changes. This chapter is about the second possibility. Representation learning matters because modern machine learning increasingly depends on reusable internal structure: embeddings that preserve useful geometry, pretrained encoders that transfer across tasks, and large foundation-model workflows that shift effort from training from scratch to adapting what has already been learned. The central question is not whether reuse is fashionable, but what exactly is being reused, how much adaptation it requires, and what evidence shows that the reuse is real rather than only hidden inside a powerful downstream model. By the end of the chapter, the reader should be able to judge when a learned representation is genuinely reusable, how to test that reuse, and when adaptation changes the claim being made.
...
The chapter extends the book's representation question from one task to many, connects baseline discipline to frozen probes and shallow adaptation, and keeps the evidence question visible by asking whether any downstream gain came from reused structure, from extra adaptation capacity, or from better external evidence. Its running artifact is a reuse and adaptation plan: state what pretrained object is being reused, what first clean diagnostic isolates reuse, what shallowest adaptation is justified, what external evidence the workflow still needs, and what alternative explanation could fake the gain.

\begin{learninggoals}
\item Explain representation learning as a change of coordinates that can make later tasks easier.
\item Distinguish reusable representations, embeddings, probes, adaptation, and task-specific heads.
\item Compare self-supervised, contrastive, masked-prediction, and transfer-learning pressures by what structure they expose.
\item Use frozen-feature and shallow-adaptation tests to ask whether reuse is actually helping.
\item Analyze foundation-model workflows in terms of reused structure, adaptation capacity, evidence, and scope limits.
\item Write a reuse and adaptation plan for a pretrained representation.
\end{learninggoals}

\begin{decisionbox}
\textbf{Core path through the chapter.} In a first serious pass, keep the spine narrow: reusable space, training pressure that created it, frozen probe, shallowest justified adaptation, and reuse plan. Attention, transformers, retrieval, prompting, and fine-tuning are important because they change the reuse claim; they should not obscure the diagnostic question of what is actually being reused.
\end{decisionbox}
...
Invest in reusable representation learning when the raw input is too entangled for simple task-specific modeling and when the learned space is likely to be reused across tasks, domains, or limited-label settings. A sensible workflow is to identify the source representation, test it first with a frozen-feature or linear-probe diagnostic, compare against a retrieval-only or prompting-only baseline when relevant, choose the shallowest adaptation that still addresses label budget and domain shift, and write down alternative explanations for any gain.
...
A useful first mental model is to treat representation learning as a change of coordinates: data are mapped into a space where the structure needed for downstream tasks becomes easier to separate, compare, or read out. It is also the mental model that holds the rest of the chapter together: if the coordinates improve, simple downstream readouts, transfer diagnostics, and later adaptation should all become easier for a reason we can inspect.
...
Not every learned representation transfers to new tasks. A representation is \emph{reusable} in a meaningful sense only when several conditions hold at once. It should capture the right invariances, so that lighting, paraphrase, nuisance background, or other label-preserving changes do not destroy the relevant structure. It should preserve the information that a new task still needs, rather than compressing so aggressively that fine distinctions disappear. It should tolerate some mismatch between the source objective and the target task, because pretraining and downstream use are never exactly the same problem. And its adaptation cost should remain contained: reuse is most valuable when a small labeled set, a linear probe, or a lightweight adapter is enough to unlock the target signal rather than requiring a full relearning of the representation.
...
Once objects are represented as vectors, we need a way to compare them. For nonzero vectors, a standard choice is cosine similarity:
\[
...
The theorem does not say every useful embedding is a random projection, and it does not replace task-specific representation learning. It does justify a central geometric intuition of the chapter: moderately low-dimensional spaces can still preserve many of the pairwise relationships that downstream retrieval, clustering, or probing depend on.
\end{advancednote}
...
Self-supervised learning creates prediction problems from structure already present in the input. Common patterns include predicting masked or missing parts of a sequence, reconstructing a clean example from a corrupted one, matching two augmented views of the same underlying object, and predicting future segments of time-series or audio data.
...
Reconstruction provides another form of self-supervised pressure. Rather than predicting masked tokens or contrasting augmented views, an autoencoder learns to compress and reconstruct its input, keeping only the structure needed to recover the original signal.
...
If the input is 1000-dimensional and the bottleneck is 10-dimensional, the encoder must compress by a factor of 100, creating strong pressure to preserve only the most useful structure. If the bottleneck is 900-dimensional, the model can nearly copy the input, learning little of value. The effective constraint is the ratio of bottleneck size to input size. Denoising and variational objectives add further pressure against trivial solutions.
\end{example}
...
Scale helps only when the self-supervised objective encourages structure that aligns with downstream tasks. A pretraining signal that rewards surface-level statistics may scale without producing representations that transfer. The question is therefore not just ``how much data?'' but ``does the training pressure reward the right invariances?''
...
Transfer learning makes reuse operational, but it still leaves one diagnostic question unanswered: is the useful structure already present in the frozen representation, or does the downstream model have to build it again?
...
The simplest reuse workflow is to pretrain an encoder on a broad source objective, freeze or partially freeze it, train a smaller downstream model on the target task, and only then decide whether some or all of the encoder should be fine-tuned.
...
So far the chapter has described reusable structure abstractly. The next practical question is whether that structure is already accessible enough to help on a new task. Before adapting a pretrained model, ask what structure is being reused, how much of it stays frozen, what downstream head or adaptation method is being added, and what evidence would show that reuse rather than downstream complexity is doing the real work.

\section{Linear Probe as the First Diagnostic}

A linear probe is a deliberately simple downstream model, often just a linear classifier or regressor trained on frozen embeddings. Following the probe tradition used to inspect intermediate representations, it is useful precisely because it separates two questions without hiding them behind a powerful downstream head \citep{alain2016probes}: did the representation itself capture useful information, or would the downstream model need to create that structure on its own?
...
The previous section argued for probing first. This section is the chapter's single adaptation ladder: first run the clean diagnostic, then choose the shallowest adaptation that the evidence justifies. The choice of adaptation strategy is not only a computational decision. It is an experimental claim about the quality and fit of the pretrained representation. Each strategy implies a different answer to the question: ``how much of the pretrained representation do I trust for this new task?''
...
Between frozen probes and full fine-tuning lies \emph{few-shot learning}: adapting a model with very few labeled examples, often via prompt engineering or lightweight adapters. The evaluation discipline is the same---only the label budget and adaptation mechanism differ.
...
The judgment rule is to start with the linear probe. If the probe achieves strong performance, the pretrained representation is already a strong candidate for the task and deeper adaptation may add little. If the probe performs poorly, partial or full fine-tuning may be warranted, but one should then ask \emph{why} the probe failed. Was the pretraining task too different? Is the target task genuinely out-of-distribution? Is the labeled set too small for fine-tuning to help? The probe failure is a diagnostic, not just a prompt to try the next strategy in the table.

\section{Adaptation Depth: Scenario Guide}

The previous section named the options. This short scenario guide turns the same menu into a decision rule rather than a second list of methods. Once a representation is shown to contain useful information, the next question is how much of it should be allowed to move.
...
\begin{theorem}[Softmax attention produces a convex combination of value vectors]
...
If the same tokens are permuted, attention weights may change because each row is computed from token content together with whatever positional or order information the model receives. Attention is therefore not only a bag of pairwise similarities; it is a context-dependent calculation over represented tokens in positions.
\end{example}
...
The previous sections described a mechanism and an architecture. This section changes level again, from model internals to engineering workflow. The defining shift is not just model size. It is workflow. Instead of training one model for one narrow task, a foundation-model workflow usually collects broad data, chooses a large-scale pretraining objective, trains a reusable model, adapts or prompts that model for many narrower tasks, and then evaluates the resulting behavior carefully under real deployment conditions.
...
The risks are equally important. Training data may be opaque or poorly documented, broad competence can mask brittle failure modes, inherited biases and harmful associations may transfer downstream, large compute cost can make experimentation uneven and less transparent, and users may overtrust fluent output as if it were verified knowledge.
...
The recurring mistakes here are all versions of one deeper error: confusing reusable behavior with justified evidence about why that behavior exists. A representation can be useful across tasks without being task-free magic; embedding neighbors are model-dependent summaries rather than ground-truth meaning; skipping the linear probe hides the basic accessibility question; self-supervision does not automatically solve label scarcity unless its training pressure matches the downstream need; transfer gains should be attributed carefully across representation, retrieval, prompting, and fine-tuning; attention maps are only diagnostic evidence; and scale does not remove the need for controlled evaluation.

\section{Looking Ahead}

This chapter framed AI around reusable representations: embeddings, probes, transfer, attention, and foundation-model workflows. The next chapter turns reusable representations toward generation: how latent spaces can be sampled, interpolated, and audited when the system produces new data rather than only describing existing data.

\section*{Chapter Summary}

This chapter shifted the unit of value from one-task accuracy to reusable structure. It argued that a learned representation is valuable when it preserves the right information, captures the right invariances, and remains useful under a lightweight downstream readout or adaptation strategy. Embeddings, probes, attention, and foundation-model workflows all fit into that same logic: progress is not just solving today's task, but learning a space that makes future tasks easier for reasons we can inspect.
