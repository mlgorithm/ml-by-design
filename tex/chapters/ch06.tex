...
This chapter asks what changes when learned features become reusable across tasks. After reading it, students should distinguish probes, retrieval, partial tuning, and full fine-tuning as different interventions with different claims. The reuse and adaptation plan extends the neural training report from one-task learning into transfer and foundation-model workflows.
\end{whychaptermatters}

The previous chapter showed how a network can learn useful internal features for one task. The next question is what happens when those features survive beyond that task.

Sometimes a model is valuable not only because it solves today's problem, but because it learns a space that makes tomorrow's problems easier. The same representation might support classification, retrieval, clustering, or later adaptation with fewer labels. Foundation-model workflows push that idea further by pretraining for reuse first and specializing later \citep{bengio2013rep,mikolov2013,peters2018elmo,oord2018cpc,vaswani2017,devlin2019,bommasani2021}.

\begin{problemsolvinglens}
When a pretrained model helps on a new task, ask what is actually doing the work: the reusable representation, fresh external evidence, or downstream adaptation.
\end{problemsolvinglens}

\section{Opening Dilemma: Solve One Task, or Learn a Reusable Space?}

Suppose you can train one model from scratch for one task, or start from a representation that has already seen broad data and may help on several tasks. That second option is not always worth the extra machinery, but when labels are scarce or the same inputs will be reused in many ways, it can change what counts as progress.

That is the opening dilemma of the chapter. Should we solve one task directly, or should we spend effort learning a reusable space that may pay off across tasks, domains, or label budgets?

The answer depends on what the reusable space really buys us. A model trained only for one classifier may work well today and still throw away value that could have helped retrieval, clustering, transfer, or later fine-tuning. A reuse-first workflow tries to keep that value.
...
The practical payoff is a reuse and adaptation plan: what is being reused, what is being changed, and what evidence would justify the transfer claim.

\begin{practicalnote}
A good first pass is simple: identify the source representation, test it with frozen features or a linear probe (a simple linear classifier trained on frozen embeddings; see Section~\ref{sec:linear-probe}), compare against a clean baseline when that comparison matters, and only then decide how much adaptation is worth doing.
\end{practicalnote}

\begin{thinkingtool}
\textbf{Reuse and adaptation plan.} A clean transfer claim separates five things: what pretrained object is being reused, what first diagnostic isolates reuse, what adaptation depth is justified, what external evidence the workflow still needs, and what alternative explanation could fake the gain.
\end{thinkingtool}

\begin{corepathbox}
\textbf{Core path.} Learn the reuse logic first: representation as change of coordinates, what makes a space reusable, probe first, then compare prompting, retrieval, and tuning by what changed.

\textbf{Enrichment.} Attention mechanics, broader pretraining taxonomies, and larger foundation-model workflow detail deepen the story, but the core undergraduate gain is being able to say where a transfer improvement came from and what it still does not prove.
\end{corepathbox}
...
The whole discussion turns on one question:
...
\textbf{Task mismatch tolerated.} The pretraining task and the new task are never exactly the same. A language representation pretrained on predicting masked words may not encode document layout, while a visual representation pretrained only for object identity may not preserve the spatial cues needed for scene understanding. Reuse is easier when the pretraining task was broad enough to touch the relevant signals and when the mismatch between source and target objectives is modest.
...
\begin{tikzpicture}[
  ...
  ebox/.style={draw=c7encBlue, fill=c7encBlueBg, rounded corners=4pt,
               align=center, minimum height=1.05cm, text width=2.1cm,
...
\node[ebox] (enc) at (3.3,0) {encoder};
\node[ebox] (emb) at (6.6,0) {embedding\\vector};
\draw[arr] (raw) -- (enc);
\draw[arr] (enc) -- (emb);

%% ── Fan-out point ───────────────────────────────────────────
\coordinate (fan) at (6.6,-1.3);
\draw[c7encBlue, line width=1.1pt] (emb.south) -- (fan);
\filldraw[c7encBlue] (fan) circle (2.5pt);

%% ── Bottom row: downstream tasks ───────────────────────────
\node[dbox] (probe) at (2.4,-2.8) {probe};
\node[dbox] (retr)  at (5.8,-2.8) {retrieve /\\cluster};
\node[dbox] (ft)    at (9.0,-2.8) {adapt};
...
%% ── Labels ──────────────────────────────────────────────────
\node[font=\footnotesize, c7encBlue, anchor=south] at (3.3,0.7)
    {\textbf{learn once}};
\node[font=\footnotesize, c7downGreen, anchor=north] at (5.8,-3.55)
    {\textbf{reuse downstream}};
\end{tikzpicture}
...
\begin{advancednote}
Cosine similarity and Euclidean distance answer different questions. Euclidean distance cares about absolute position and scale. Cosine cares about direction. In some applications the distinction is minor; in others it changes the neighbor ranking substantially.
\end{advancednote}
...
The next question is where reusable structure comes from in the first place. These workflows make the most sense when they are understood as different kinds of \emph{training pressure} on the learned space, not as disconnected method names. By training pressure we mean the structural demands that a training objective places on the representation---what the model is forced to organize in order to reduce its loss. Different objectives really do produce different reuse profiles. Supervised pretraining often optimizes one label space well and can transfer strongly when the downstream task is close. Self-supervised objectives such as masked prediction, contrastive alignment (which trains the model to pull representations of similar items closer together and push dissimilar items apart), and self-distillation instead force the model to preserve context, invariance, or cross-view agreement that may transfer more broadly \citep{bengio2013rep,devlin2019,chen2020simclr,he2020moco,grill2020byol,caron2021dino}. Transfer and adaptation then test how much of that reusable structure actually survives contact with a new task.
...
\begin{tikzpicture}[x=0.9cm, y=1cm, font=\small,
  sbox/.style={draw=black!55, fill=black!4, rounded corners=4pt,
               align=center, minimum height=1.1cm, text width=2.1cm,
               font=\small, line width=0.8pt},
  ...
  lossbox/.style={draw=black!60, fill=yellow!8, rounded corners=4pt,
               align=center, minimum height=1.3cm, text width=2.2cm,
               font=\small, line width=0.8pt},
...
%% ── Fork point ──────────────────────────────────────────────
\coordinate (fork) at (1.5,0);
\draw[black!65, line width=1.1pt] (raw.east) -- (fork);
\filldraw[black!55] (fork) circle (2.5pt);

%% ── Top path (model) ───────────────────────────────────────
\node[mbox] (view) at (4.0, 1.5) {corrupted /\\augmented view};
\node[mbox] (enc)  at (6.8, 1.5) {encoder $\rightarrow z$};
\draw[arr] (fork) -- (view.west);
\draw[arr] (view) -- (enc);

%% ── Bottom path (target) ───────────────────────────────────
\node[tbox] (target) at (4.0,-1.5) {target /\\paired view};
\draw[arr] (fork) -- (target.west);

%% ── Right: loss ─────────────────────────────────────────────
\node[lossbox] (loss) at (8.7, 0) {pretext loss:\\predict or\\compare};
%% Top path feeds into loss from above
\draw[-Latex, line width=1.1pt, draw=c7ssBlue]
    (enc.east) -- (8.7, 1.5) -- (loss.north);
%% Bottom path feeds into loss from below
\draw[-Latex, line width=1.1pt, draw=c7ssGreen]
    (target.east) -- (8.7,-1.5) -- (loss.south);

%% ── Path labels ─────────────────────────────────────────────
\node[font=\footnotesize\bfseries, c7ssBlue] at (5.2, 2.25) {model path};
\node[font=\footnotesize\bfseries, c7ssGreen] at (5.2,-2.25) {target path};
\end{tikzpicture}
...
\]
where $f_\theta$ is the encoder and $g_\theta$ is the decoder (here $\theta$ denotes all model parameters jointly, both encoder and decoder). The loss is small only when the latent representation preserves enough structure to rebuild the input faithfully.
\end{mathlens}
...
The main practical advantage of self-supervision is that unlabeled data is usually much more abundant than carefully labeled data. If the pretraining objective is chosen well, large amounts of raw data can be turned into representational pressure before a downstream label set even exists. This is not a bigger-is-better claim. It explains why reuse-first workflows became practical at all.
...
Scale helps because raw data is plentiful, but that should not invite a lazy conclusion. Self-supervision scales only when the objective teaches the invariances or contextual structure the downstream task actually needs. More unlabeled data is not a substitute for a reuse audit \citep{chen2020simclr,devlin2019}.
...
Once a reusable space exists, the next issue is whether it actually helps on a new task. That is where transfer learning begins.
...
Transfer is not a victory lap after pretraining. It is the first serious test of whether the representation contains linearly or cheaply accessible structure for the new task. A useful heuristic from earlier transfer work is that lower layers often transfer more generally while upper layers become more task-specific, but that pattern is not a law: modern architectures and tasks can blur it, and stronger source models often transfer better without doing so uniformly across downstream tasks \citep{yosinski2014transfer,kornblith2019transfer}.
...
\begin{practicalnote}
Keep one downstream target in mind through the rest of the chapter: a university support assistant that must classify incoming messages, retrieve current policy passages, and either answer or escalate. The next sections revisit that same target under frozen probes, deeper heads, tuning, prompting, and retrieval so that the differences feel like one workflow rather than a list of adaptation options.
\end{practicalnote}
...
\begin{practicalnote}
When transfer fails, diagnose the mismatch before tuning harder. Ask whether the source data lacked the target nuisance factors, whether the label meaning shifted, whether the tokenizer or sensor changed what counts as signal, and whether the target task needs information the source objective never had to preserve. These are different failure stories, so they deserve different next comparisons.
\end{practicalnote}

\section{Probe, Freeze, Partial Tune, or Full Tune?}

Probing comes first, but it is not the only option. The broader menu of adaptation choices is an experimental claim about the quality and fit of the pretrained representation, not only a computational decision. Each strategy implies a different answer to the question: ``how much of the pretrained representation do I trust for this new task?''

\textbf{Linear probe (frozen encoder).} Train only a linear head on top of the frozen pretrained features. This strategy makes the strongest claim about the representation: the task-relevant information is already linearly accessible in the frozen space. The probe is also the cleanest first diagnostic: if it works well, the frozen representation is reusable for that target under a simple linear readout. If it works poorly, the result says the frozen space is not enough on its own, but it does not by itself tell us whether the issue is missing signal, a weak probe setup, or a need for more adaptation.
...
The judgment rule is to start with the linear probe. If the probe achieves strong performance, the pretrained representation is already carrying a large share of the task-relevant information, and further adaptation may be unnecessary. If the probe performs poorly, that is a reason to inspect the representation and the probe setup before moving to deeper tuning. The failure is a diagnostic, not just a prompt to try the next strategy in the table.

\section{Adaptation Depth: Freeze, Partial Tuning, Full Tuning}

That menu becomes a simple rule: start with the cheapest test that tells you whether the pretrained space already exposes the signal, then move deeper only when the evidence says the frozen or lightly adapted model is not enough.

The modern design space is wider than freeze versus full fine-tuning. Gradual-unfreezing strategies such as ULMFiT made this visible early in NLP, and later adapter modules and low-rank updates made the same point at larger scale: teams can specialize a pretrained model while moving only part of it \citep{howard2018ulmfit,houlsby2019adapters,hu2021lora}. These methods can lower compute and preserve more of the original model, but they still need the same question answered: did the gain come from reuse, or from extra task-specific capacity?
...
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{L{2.0cm}L{3.28cm}L{3.72cm}}
\toprule
Surface token & Local context & Contextual reading \\
\midrule
\texttt{bank} & \texttt{the bank approved the loan} & financial institution participating in a lending event \\
\texttt{bank} & \texttt{the river reached the bank} & physical boundary of the river rather than a financial actor \\
\bottomrule
\end{tabular}
\caption{The token type can stay the same while the contextual representation changes. Attention makes that shift operational by letting the token borrow information from surrounding words.}
\label{tab:token-before-after-context}
\end{table}
...
In the transformer formulation, every element in the sequence has its own query, key, and value vectors. For a given element, its query vector~$q$ is compared against the key vectors~$k_j$ of all elements (including itself), and the result is used to weight the corresponding value vectors~$v_j$. The attention weights are computed by normalizing the full list of query-key similarities:
\[
...
By this point, the broad pattern should feel familiar: pretrain once on a large and varied source, then reuse the result through prompting (structuring the input to steer the model's output at inference time without changing its parameters), retrieval, or downstream adaptation. Large language models and multimodal encoders made that reuse-first pattern especially visible at scale.

The term \emph{foundation model} is used for a broadly pretrained model built for that kind of reuse across many downstream tasks, often across domains or modalities.
...
Transfer is not one thing. Prompting changes the instruction, retrieval changes the accessible evidence, and fine-tuning changes the predictor itself. The cleanest claim is the one that stays at the level of the intervention that actually changed.
\end{minimumtakeaway}
...
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{L{2.18cm}L{2.23cm}L{1.95cm}L{2.26cm}}
\toprule
Intervention on the same support task & What changed most & Strongest defensible claim & What it still does not prove \\
\midrule
Prompting only & answer format and local behavior policy & behavior framing improved & no new evidence entered and no durable task adaptation was shown \\
Retrieval augmentation & current policy evidence at inference time & evidence access improved & the pretrained representation itself did not necessarily improve \\
Frozen linear probe & simple readout on frozen embeddings & task signal is linearly accessible in the reused space & robustness under domain shift is still untested \\
Frozen deeper head & extra downstream readout capacity & the frozen space may still be useful with a richer head & gains may come from the head rather than from clean reuse \\
Partial fine-tuning & top layers adapt to local phrasing & modest task-specific adjustment was helpful & reuse is now less cleanly isolated than under a probe \\
Full fine-tuning & more of the model is reshaped for the target & target labels and broader adaptation improved task fit & the gain may come mainly from downstream training rather than from reusable structure \\
\bottomrule
\end{tabular}
\caption{One downstream task can look better for many different reasons. This is why adaptation choice is also a claim audit: each intervention changes what part of the observed gain should be attributed to reuse, evidence, or new task-specific training.}
\label{tab:one-task-many-claims}
\end{table}
...
Once reuse enters the picture, a strong result can come from several different places: the pretrained representation, fresh retrieved evidence, prompting, or downstream tuning. If those sources are not separated, the final claim becomes blurry. The plan below is a compact way to state what is being reused, what is being changed, and what evidence would show that reuse is actually helping.
...
\begin{thinkingtool}
\textbf{Reuse and adaptation plan.} A strong plan makes explicit what is being reused, what is being changed, the first clean diagnostic of reuse, and what evidence would justify a strong transfer claim.
\end{thinkingtool}
...
Embeddings, probes, transfer, attention, and foundation-model workflows all treat AI as a question of reusable representations. The next chapter grounds that idea in vision, where the reuse question becomes concrete: which visual changes should a model ignore, which should it preserve, and what augmentations or architectures actually encode that judgment?

The reuse and adaptation plan from this chapter becomes the starting document for Chapter 8's vision design and stress-test card: reusable features still have to be audited against domain-specific invariances and deployment conditions.

\begin{chaptersummary}
\item The central question here is when it is worth learning a reusable representation instead of solving only one task directly, and how to tell that the reusable representation is what is helping.
...
\item A pretrained encoder is evaluated on the same downstream task in four ways: frozen features plus linear probe gives $84\%$ accuracy, frozen features plus a two-layer head gives $83\%$, full fine-tuning gives $85\%$, and training from scratch gives $71\%$. State the clearest first claim this result supports, and one thing it still does \emph{not} prove about reuse.
\item A frozen probe works well on typed course emails but fails on multilingual chat messages with slang and campus-specific abbreviations. Give two plausible reasons this transfer failed under domain shift, and name one clean next comparison before full fine-tuning.
