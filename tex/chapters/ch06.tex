...
A model can succeed on a task in two very different ways. It can learn a narrow solution tied closely to that task, or it can learn a representation that stays useful when the task changes. This chapter is about the second possibility. Representation learning matters because modern machine learning increasingly depends on reusable internal structure: embeddings that preserve useful geometry, pretrained encoders that transfer across tasks, and large foundation-model workflows that shift effort from training from scratch to adapting what has already been learned. The central question is not whether reuse is fashionable. It is what exactly is being reused, how much adaptation it requires, and what evidence shows the reuse is real rather than hidden inside a powerful downstream model. By the end of the chapter, you should be able to judge when a learned representation is genuinely reusable, how to test that reuse, and when adaptation changes the claim being made.

The shift is larger than it first appears. In classical machine learning, a model is typically trained for one task, on one dataset, with one objective. Representation learning changes the unit of value. What matters is not only whether the model solves today's task, but whether it learns a space useful for tomorrow's tasks too. Foundation-model workflows push this further by pretraining for reuse first and adapting later. The modern form of the idea draws on work on representation learning as exposed structure, contextual word representations, latent prediction, attention-based sequence modeling, bidirectional pretraining, and foundation-model analysis \citep{bengio2013rep,mikolov2013,peters2018elmo,oord2018cpc,vaswani2017,devlin2019,bommasani2021}. The chapter follows the same path: first what a reusable space is, then what training pressure creates it, then how probes and adaptation test it, and finally how attention and foundation-model workflows scale the same logic.

The chapter extends the book's representation question from one task to many, connects baseline discipline to frozen probes and shallow adaptation, and keeps the evidence question visible by asking whether any downstream gain came from reused structure, extra adaptation capacity, or better external evidence. The running artifact is a reuse and adaptation plan that states what pretrained object is being reused, what first clean diagnostic isolates reuse, what shallowest adaptation is justified, what external evidence the workflow still needs, and what alternative explanation could fake the gain.
...
The previous chapter ended by showing that neural networks can learn internal features rather than relying only on hand-built ones. This chapter begins where that idea becomes strategically important. The real value of a model is sometimes not that it solves one task well, but that it learns a space in which many related tasks become easier.

That is the chapter's opening dilemma. Should we solve a task directly from scratch, or invest in learning a reusable representation that might help across tasks, domains, or label budgets?
...
\]
where $p$ is the position and $i$ indexes the dimension. Modern variants prefer learned positional embeddings or rotary position embeddings (RoPE), which encode position multiplicatively inside the attention computation rather than additively at the input. The architectural choice matters more for long contexts (where extrapolation beyond the longest training sequence is needed) than for short ones.
...
The recurring mistakes are all versions of one deeper error: confusing reusable behavior with justified evidence about why that behavior exists. A representation can be useful across tasks without being task-free magic. Embedding neighbors are model-dependent summaries, not ground-truth meaning. Skipping the linear probe hides the basic accessibility question. Self-supervision does not automatically solve label scarcity unless its training pressure matches the downstream need. Transfer gains must be attributed carefully across representation, retrieval, prompting, and fine-tuning. Attention maps are diagnostic evidence, nothing more. And scale does not remove the need for controlled evaluation.
...
This chapter shifted the unit of value from one-task accuracy to reusable structure. A learned representation is valuable when it preserves the right information, captures the right invariances, and remains useful under a lightweight downstream readout or adaptation strategy. Embeddings, probes, attention, and foundation-model workflows all fit the same logic: progress is not just solving today's task, but learning a space that makes future tasks easier for reasons we can inspect.
