...
\begin{learninggoals}
\item Explain when a probabilistic model is needed instead of a single point prediction.
\item Distinguish generative and discriminative views and the task consequences of each.
\item Treat latent variables as hidden structure inferred from observed data.
\item Describe the EM algorithm and Gaussian mixtures as a workflow for soft latent assignments.
\item Use Bayesian vocabulary---prior, likelihood, posterior, MLE, and MAP---to explain uncertainty-aware modeling choices.
\item Distinguish aleatoric uncertainty from epistemic uncertainty and connect the distinction to data collection and review.
\item Use Naive Bayes as a transparent probabilistic baseline whose probabilities still need calibration.
\item Construct an uncertainty audit card that documents what the model knows it does not know.
\item Treat Bayesian linear regression and Gaussian processes as extension lenses for input-dependent uncertainty unless the course emphasizes probabilistic modeling.
\end{learninggoals}
...
The shift from weights to functions is not just philosophical. It changes what the prior encodes. In Bayesian linear regression, a prior on $\mathbf{w}$ expresses ``parameters should be small.'' In a GP, the prior is specified by a \emph{kernel} $k(\mathbf{x}, \mathbf{x}')$ that measures similarity between inputs, and the implicit belief is ``nearby inputs should produce nearby outputs.'' The choice of kernel therefore encodes what ``nearby'' means. A squared-exponential kernel assumes the underlying function is smooth; a Mat\'ern kernel allows rougher behavior; a periodic kernel expresses the belief that the function repeats with a known period. Every kernel is a hypothesis about the function family the data lives in.

\textbf{Predictive distribution.} For a new input $\mathbf{x}_*$, the \emph{standard GP-regression} setting---Gaussian likelihood with observation noise variance $\sigma_n^2$ and a kernel with fixed hyperparameters---gives a predictive distribution that is Gaussian with mean $\mu(\mathbf{x}_*)$ and variance $\sigma^2(\mathbf{x}_*)$. Both depend on the training data and the kernel. Three properties make the variance particularly useful for reliability work. First, it is \emph{input-dependent}: the GP is confident in regions where training points cluster and unconfident in regions where training points are sparse. Second, under fixed kernel and noise hyperparameters it does \emph{not} depend on the observed outputs $\mathbf{y}$: the posterior variance is a function of the training-input locations, the kernel, and the assumed noise level only. Third, for stationary kernels whose covariance decays with distance, it grows smoothly toward the prior variance as $\mathbf{x}_*$ moves far from the training data, giving an honest ``I do not know'' signal rather than a falsely confident extrapolation. Other kernels extrapolate according to their own structure.

\begin{warningbox}
The ``variance does not depend on $\mathbf{y}$'' property is a feature of the exact GP-regression formula with \emph{fixed} hyperparameters. In practice the noise variance $\sigma_n^2$ and the kernel hyperparameters (length-scales, output scale) are almost always learned from the training data---typically by maximizing the marginal likelihood $p(\mathbf{y} \mid \mathbf{X})$---and that learning couples the predictive variance to $\mathbf{y}$ indirectly. The geometry-of-inputs intuition is right for a fixed kernel and is a useful teaching lens for why GPs extrapolate honestly, but a deployed GP whose hyperparameters were fitted to data does carry output information in its uncertainty. Non-Gaussian likelihoods (GP classification, count data) break the closed-form Gaussian posterior altogether and require approximate inference.
\end{warningbox}
\end{advancednote}
