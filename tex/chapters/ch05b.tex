...
\begin{prereqbridge}
Review Bridge~II if conditional probability, likelihood, expectation, or variance slows the reading down. The core path in this chapter is design fluency with probabilistic structure and uncertainty; the EM proof and deeper Gaussian-process derivations can be treated as extension work in a first course.
\end{prereqbridge}

\section{Why Probabilistic Thinking Matters}

Many models encountered so far produce a single operational output for each input: a score, a class label, or a thresholded decision. Some of them, such as logistic regression or softmax classifiers, can also produce class probabilities. The issue is not that previous models were never probabilistic; the issue is that their probabilities were often treated only as a way to choose a label, rather than as quantities to calibrate, audit, or propagate into downstream decisions.

It is useful to keep three levels separate: point labels or scores; class-probability models such as logistic regression; and full predictive or generative distributions that model richer uncertainty about labels, inputs, or latent structure. This chapter focuses on the latter two levels and asks how uncertainty should be represented and used.

Probabilistic models take a different stance: the output is a \emph{distribution}, not merely a point. Instead of asking only ``what is the predicted class?'' a probabilistic model asks ``what distribution over possible outcomes is being represented, and how spread out is that distribution?'' This shift has practical consequences.
...
\item \emph{Outlier detection.} Generative models can support density-based checks for inputs that are unlikely under the learned distribution, but this must be validated because likelihood can be misleading out of domain. Plain discriminative classifiers have no built-in input-density model; they need an added rejection rule, uncertainty check, or out-of-distribution detector.
...
\item Generate the data point $x$ from a Gaussian with mean $\mu_k$ and covariance $\Sigma_k$.
\end{enumerate}
...
The model's parameters are the mixing weights $\{\pi_k\}$, the means $\{\mu_k\}$, and the covariances $\{\Sigma_k\}$. The latent variable for each data point is $z_i \in \{1, \ldots, K\}$: which component generated it.

\subsection{Soft vs.\ Hard Clustering}

$k$-means, encountered in earlier chapters, assigns each data point to exactly one cluster. GMMs generalize this: each data point has a \emph{soft assignment}---a probability distribution over clusters. After fitting the model, data point $x_i$ belongs to cluster $k$ with probability:
\[
...
\begin{practicalnote}
GMMs connect to $k$-means in a special case: when all covariance matrices are $\sigma^2 I$ (spherical, equal variance) and $\sigma \to 0$, the soft assignments become hard assignments and the EM algorithm (below) becomes the $k$-means update rule. In that sense, $k$-means can be viewed as the hard-assignment limiting case of EM for a constrained spherical GMM, even though it is used as a distinct algorithm in practice.
\end{practicalnote}
...
Each $\mu_k$ is a weighted average of the data points, where the weights are the soft assignments. Each $\Sigma_k$ is a weighted covariance. Each $\pi_k$ is the average responsibility of component $k$.

\begin{example}[One EM Pass in One Dimension]
Suppose three points are \(x=\{0,2,8\}\), and a two-component mixture starts with rough means \(\mu_1=1\) and \(\mu_2=7\). After the E-step, imagine the responsibilities for component 1 are
\[
\gamma_{\cdot 1}=(0.90,\ 0.80,\ 0.05),
\]
so the responsibilities for component 2 are \((0.10,0.20,0.95)\). The M-step updates the first mean as a responsibility-weighted average:
\[
...
=\frac{8.0}{1.25}=6.40.
\]
This is the whole EM rhythm: infer soft membership with the current parameters, then recompute parameters as weighted statistics under those memberships.
\end{example}
...
A course-support team has 5,000 student messages with no labels. The GMM does not operate on the raw strings directly: each message is first embedded or featurized into a numeric space where Gaussian clusters are at least a defensible approximation. That representation choice determines whether the mixture assumption makes sense.
...
All models fitted so far have used \emph{maximum likelihood estimation} (MLE): find the parameters $\theta$ that maximize $P(\text{data} \mid \theta)$. MLE treats parameters as unknown but fixed quantities and finds the single best value.
...
The prior is typically Gaussian: $P(w) = \mathcal{N}(0, \sigma_w^2 I)$. Given data $(X, y)$ and an assumed noise model $y_i = w^\top x_i + \epsilon$ where $\epsilon \sim \mathcal{N}(0, \sigma^2)$, the posterior is also Gaussian (a property of conjugate priors):
\[
P(w \mid X, y) = \mathcal{N}(\mu_{\text{post}}, \Sigma_{\text{post}})
\]

The posterior mean $\mu_{\text{post}}$ is the MAP estimate (equivalent to ridge regression). But the posterior covariance $\Sigma_{\text{post}}$ is new: it tells us how uncertain we are about the weights. Directions in parameter space where the data provides strong evidence have low posterior variance; directions where the data is sparse have high posterior variance.

\textbf{Predictive distribution.} For a new input $x_*$, the prediction is not a single value but a distribution:
\[
P(y_* \mid x_*, X, y) = \mathcal{N}(\mu_{\text{post}}^\top x_*, \; x_*^\top \Sigma_{\text{post}} x_* + \sigma^2)
\]

The prediction variance has two terms. The first, $x_*^\top \Sigma_{\text{post}} x_*$, reflects \emph{epistemic uncertainty}---uncertainty about the model parameters that would shrink with more data. The second, $\sigma^2$, reflects \emph{aleatoric uncertainty}---inherent noise in the data that cannot be reduced by collecting more data. This decomposition is one of the most important ideas in probabilistic modeling.
...
\begin{advancednote}
Bayesian linear regression reasons about a distribution over weights. A \emph{Gaussian process} (GP) reasons about a distribution over \emph{functions} directly, without committing to a parametric form. Given training inputs and outputs, a GP returns, for every possible new input, a Gaussian distribution over the function value at that input. The predictive mean gives the best point estimate; the predictive variance says how much the GP is willing to commit to that estimate at that particular location.

The shift from weights to functions is not just philosophical. It changes what the prior encodes. In Bayesian linear regression, a prior on $w$ expresses ``parameters should be small.'' In a GP, the prior is specified by a \emph{kernel} $k(x, x')$ that measures similarity between inputs, and the implicit belief is ``nearby inputs should produce nearby outputs.'' The choice of kernel therefore encodes what ``nearby'' means. A squared-exponential kernel assumes the underlying function is smooth; a Mat\'ern kernel allows rougher behavior; a periodic kernel expresses the belief that the function repeats with a known period. Every kernel is a hypothesis about the function family the data lives in.

\textbf{Predictive distribution.} For a new input $x_*$, the \emph{standard GP-regression} setting---Gaussian likelihood with observation noise variance $\sigma_n^2$ and a kernel with fixed hyperparameters---gives a predictive distribution that is Gaussian with mean $\mu(x_*)$ and variance $\sigma^2(x_*)$. Both depend on the training data and the kernel. Three properties make the variance particularly useful for reliability work. First, it is \emph{input-dependent}: the GP is confident in regions where training points cluster and unconfident in regions where training points are sparse. Second, under fixed kernel and noise hyperparameters it does \emph{not} depend on the observed outputs $y$: the posterior variance is a function of the training-input locations, the kernel, and the assumed noise level only. Third, for stationary kernels whose covariance decays with distance, it grows smoothly toward the prior variance as $x_*$ moves far from the training data, giving an honest ``I do not know'' signal rather than a falsely confident extrapolation. Other kernels extrapolate according to their own structure.

\begin{warningbox}
The ``variance does not depend on $y$'' property is a feature of the exact GP-regression formula with \emph{fixed} hyperparameters. In practice the noise variance $\sigma_n^2$ and the kernel hyperparameters (length-scales, output scale) are almost always learned from the training data---typically by maximizing the marginal likelihood $p(y \mid X)$---and that learning couples the predictive variance to $y$ indirectly. The geometry-of-inputs intuition is right for a fixed kernel and is a useful teaching lens for why GPs extrapolate honestly, but a deployed GP whose hyperparameters were fitted to data does carry output information in its uncertainty. Non-Gaussian likelihoods (GP classification, count data) break the closed-form Gaussian posterior altogether and require approximate inference.
\end{warningbox}
\end{advancednote}

\begin{thinkingtool}
\textbf{Kernel as a hypothesis.} Every GP kernel is a claim about which inputs should share information. When choosing a kernel, ask: if two inputs are ``similar'' under this kernel, does that similarity predict anything the task actually cares about? A spatial kernel over GPS coordinates may be right for crowd density but wrong for student-message similarity, where lexical or semantic distance matters more than Euclidean distance in some feature space.
\end{thinkingtool}

\begin{example}[Gaussian Process for Pedestrian Counts with Sparse Sensors]
The campus shuttle team deploys counting sensors at only eight locations along the main walking corridor, but the shuttle-routing system needs density estimates at twenty-five candidate stop locations. A GP with a squared-exponential kernel over walkway position interpolates between the eight measurements. Near each sensor the predictive mean tracks the observed count and the predictive variance is small. Midway between sensors the mean is a smooth interpolation and the variance grows. At the two ends of the corridor, outside the sensor placements, the variance approaches the prior variance, signaling that the estimate there is essentially a guess. The routing system uses the predictive variance directly: it proposes a new stop only at candidate locations where the predicted density is high \emph{and} the predictive variance is low, avoiding locations where the model is merely guessing confidently.
\end{example}

\begin{practicalnote}
In standard GP regression with a Gaussian likelihood and fixed hyperparameters, inference is exact and non-parametric, but computing the predictive distribution requires solving an $n \times n$ kernel system at cost $O(n^3)$ where $n$ is the number of training points. GPs are therefore most useful when data is scarce (hundreds to a few thousand points) and calibrated uncertainty is valuable---spatial and temporal interpolation, small-sample scientific regression, and Bayesian optimization of expensive hyperparameters or experiments. For large-scale problems, sparse GP approximations or neural-network-based uncertainty methods (ensembles, Monte Carlo dropout, or variational inference, introduced in the uncertainty section below) scale better at the cost of giving up exact posterior guarantees.
\end{practicalnote}
...
\textbf{Calibration.} As introduced in Chapter~\ref{ch:prediction-loss-decision}, a model is well calibrated when events predicted with probability $p$ occur about a fraction $p$ of the time---i.e., across all predictions assigned a probability near $0.9$, roughly $90\%$ should turn out to be positive. This is a property of the predicted probabilities themselves and is distinct from accuracy at any single threshold. Calibration can be measured with reliability diagrams and the Expected Calibration Error (ECE). Temperature scaling is a simple post-hoc calibration method: divide the logits by a scalar temperature $T$ before applying softmax, where $T$ is tuned on validation data.
...
Several mistakes recur when building probabilistic models. The first is treating all uncertainty as the same: using a single confidence score without distinguishing aleatoric from epistemic sources leads to poor decisions about when to collect more data and when to accept irreducible noise. The second is trusting uncalibrated probabilities: neural network softmax outputs are not calibrated by default, and using them as if they were leads to overconfident systems. A third is choosing $K$ for a mixture model based on aesthetics rather than evidence: use evidence such as information criteria, held-out likelihood, stability checks, and domain constraints. A fourth is assuming that the Naive Bayes independence assumption makes the model useless: it is wrong but often useful, and dismissing it means losing a fast, interpretable baseline. Finally, treating the EM algorithm as a black box without checking for convergence, sensitivity to initialization, or empty components leads to unreliable clustering.

\section{Looking Ahead}

This chapter introduced probabilistic models as tools for reasoning about distributions, latent structure, and uncertainty. Gaussian mixture models showed how latent variables and the EM algorithm can discover hidden structure. Bayesian reasoning formalized the distinction between aleatoric and epistemic uncertainty and connected regularization to prior knowledge. Gaussian processes extended that reasoning from parametric to non-parametric modeling, placing a prior on functions rather than weights so that predictive uncertainty grows honestly in regions the training data never reached. Naive Bayes demonstrated that even a model with ``wrong'' assumptions can serve as a useful, interpretable baseline. The uncertainty audit card documented what the model knows it does not know.
...
Probabilistic models reason about distributions, not just point predictions. Generative models learn the joint distribution $P(X, Y)$ and can support outlier checks, handle missing data, and generate new examples; discriminative models learn decision rules or conditional probability models for predicting $Y$ from $X$ and typically achieve higher classification accuracy. Latent variables represent hidden structure that must be inferred from observations. Gaussian mixture models use latent cluster assignments and the EM algorithm to discover soft clusters. Bayesian reasoning treats parameters as random variables with prior distributions, connects regularization to prior beliefs, and produces posterior distributions that encode uncertainty. Gaussian processes push this idea from parametric to non-parametric regression: by placing a prior on functions through a kernel, they yield predictive variance that grows where training data is sparse and shrinks where it is dense; under fixed kernel and noise hyperparameters, the latent-function posterior variance depends on the training input locations and kernel, not directly on observed output values. The posterior predictive distribution decomposes into aleatoric uncertainty (inherent noise) and epistemic uncertainty (model ignorance). Naive Bayes is a fast, interpretable generative baseline that works despite its strong independence assumption. An uncertainty audit card documents what kinds of uncertainty the model faces, how they are measured, and what downstream decisions depend on them.
...
\item How does a Gaussian process's predictive variance behave as a new input moves away from the training data, and why is this pattern more honest than a parametric model's extrapolated confidence?
