...
\textbf{Normalized Discounted Cumulative Gain (NDCG).} NDCG assigns graded relevance scores to items rather than binary relevant/irrelevant labels, and discounts items lower in the list by $1/\log_2(\text{rank} + 1)$. The score is normalized against the ideal ranking. NDCG is the most widely used ranking metric because it handles graded relevance and position discounting together.
