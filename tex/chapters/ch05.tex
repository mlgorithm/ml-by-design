...
Chapter 5 asked which kind of structure should be added next. This chapter asks what happens when the structure itself is not fully specified in advance and must instead be learned during training.
...
\begin{theorem}[Sigmoid cross-entropy produces the training signal \(\hat{p}-y\)]
For a binary classifier with output score \(s\), predicted probability \(\hat{p}=\sigma(s)\), and label \(y\in\{0,1\}\), the derivative of binary cross-entropy with respect to the score simplifies to
\[
...
Later chapters will repeatedly use multiclass classification, so it is worth seeing the standard pattern now. A multiclass output layer usually produces one raw score per class, called a \emph{logit}, before those scores are normalized into probabilities. Softmax converts those logits into probabilities:
\[
...
for each minibatch gradient grad:
    ...
    if using AdamW:
        parameters = parameters - learning_rate * weight_decay * parameters
\end{codeexample}
...
\begin{practicalnote}
Backpropagation is efficient because it requires only one forward pass and one backward pass to compute the gradient with respect to every parameter in the network. The alternative---numerical differentiation---would require a separate forward pass for each parameter, making it orders of magnitude slower for models with millions of weights.
\end{practicalnote}
...
\begin{exercises}
\item Given a learning curve where training loss decreases steadily but validation loss begins increasing after epoch 10, identify the failure type and propose two interventions.
...
\item Take the toy parabola from the gradient descent example. Compute three update steps with learning rates $\eta = 0.01$, $\eta = 0.5$, and $\eta = 1.5$. For which rate does the loss decrease fastest? For which does it diverge?
\end{exercises}
