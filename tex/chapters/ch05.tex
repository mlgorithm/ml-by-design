...
\begin{learninggoals}
\item Explain a neural network as a learned feature pipeline followed by a prediction layer.
\item Trace a forward pass, loss computation, and gradient-based update at a conceptual level.
\item Distinguish optimization failure from generalization failure during training.
\item Use debugging checks, regularization, and validation curves to make neural training inspectable.
\item Write a minimum credible neural training report that records architecture, data, optimization, and evidence.
\end{learninggoals}
...
\item \exercisetag{Calculation}{Intermediate}For the loss \(L(w)=(w-3)^2\) with initial value \(w_0=0\), compute three update steps with learning rates \(\eta = 0.01\), \(\eta = 0.5\), and \(\eta = 1.5\). For which rate does the loss decrease fastest? For which does it diverge?
\end{exercises}
