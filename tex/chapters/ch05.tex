...
\begin{learninggoals}
\item Explain when learned internal structure becomes more appropriate than hand-designed features alone.
\item Understand a neural network as a learned feature pipeline rather than as a bag of layers and tricks.
\item Compute a simple forward pass by hand and explain what each hidden unit contributes.
\item Treat losses as teaching signals, optimization as repeated correction, and backpropagation as credit assignment.
\item Explain how regularization tries to make learned representations less brittle on new data.
\item Produce a minimum credible neural training report before making a serious model claim.
\end{learninggoals}

\begin{whychaptermatters}
This chapter turns neural networks from folklore into a trainable debugging workflow. After reading it, students should separate forward pass, loss, optimization, generalization, and reporting instead of treating ``deep learning'' as one opaque step. The minimum credible neural training report extends the structure diagnosis sheet by recording what justified the move to learned representations.
\end{whychaptermatters}

The previous chapter asked what kind of structure a simple baseline was missing. Sometimes trees or geometry-based methods are enough. Sometimes they are not, because the useful intermediate features are not obvious in advance.

That is the setting where neural networks help. Instead of depending entirely on hand-built splits, distances, or interactions, they can learn internal representations through optimization.

The question underneath all of this is simple:

\begin{center}
\emph{When should structure be learned rather than engineered, and how does optimization actually teach a network to build that structure?}
\end{center}

\begin{problemsolvinglens}
When hand-engineered structure stops being enough, ask how the model can learn the intermediate features it needs and what evidence would show that the training process actually worked.
\end{problemsolvinglens}

The practical payoff is a neural result that is readable: what the network saw, how it was trained, and what evidence makes the claim believable.

\begin{thinkingtool}
\textbf{Minimum credible neural training report.} A believable neural result makes six things visible: the baseline to beat, the input representation and output type, the loss, the optimization setup, the stopping rule, and the evidence that separates optimization failure from generalization failure.
\end{thinkingtool}

\begin{decisionbox}
\textbf{Must-master path through this chapter.} On a first pass, master the learned feature pipeline, the worked forward pass, loss as teaching signal, optimization as repeated correction, backpropagation as credit assignment, generalization versus regularization, and the minimum credible training report. Treat detailed optimizer comparisons, batch-normalization mechanics, and broader capacity-regime discussion as supporting material until that core path feels stable.
\end{decisionbox}
...
\item Choose a neural network when the task likely depends on multiple layers of representation that are difficult to specify manually.
\item Prefer a simpler baseline if a linear model, tree ensemble, or hand-designed feature pipeline already solves the problem well enough.
\item Use the added flexibility only when you have enough data, compute, and evaluation discipline to justify it.
\item Decide early whether the output will be used as a score, a thresholded decision, or a calibrated probability.
\end{itemize}
\end{decisionbox}

\begin{practicalnote}
Workflow sketch: start with the strongest simple baseline, decide what structure it is missing, build the smallest network that could plausibly learn that structure, train with a validation split while watching for optimization versus generalization failure, and then report the result in a minimum credible training report. The rest of the chapter follows that order: representation, forward pass, loss, optimization, debugging, generalization, and then reporting.
\end{practicalnote}

\begin{thinkingtool}
\textbf{Starter recipe for a first small MLP.}
\begin{itemize}[leftmargin=*]
\item Normalize the inputs and choose the smallest network that could plausibly learn the missing structure.
\item Match the output layer and loss to the task before tuning anything else.
\item Hold out validation data immediately and decide what curve or metric will trigger stopping.
\item Run the tiny-batch overfit test early so optimization failures are separated from generalization failures.
\item Inspect training and validation curves before changing depth or width.
\item Change one training choice at a time and record what happened.
\end{itemize}
\end{thinkingtool}
...
\]
Here $x$ is the input vector, $w$ is a vector of weights, $b$ is a bias term, $z$ is the pre-activation score, and $\phi$ is the activation function. The output $a$ then becomes part of the input to the next layer.

This should look like an extension of Chapter 4 rather than a complete break from it. The affine part $w^\top x + b$ is still a weighted sum plus bias. What changes is that the network repeats this process across layers and inserts nonlinearities between them. Those nonlinearities allow the model to reshape the representation itself.
...
This is why activation functions matter pedagogically. Their job is not to create trivia to memorize. Their job is to make learned internal features more expressive than the weighted-sum models of Chapter 4.
...
\toprule
Quantity & Expression & Before act. & After act. \\
\midrule
...
\begin{warningbox}
A sigmoid or softmax output is a model score, not yet an action. If the model will make decisions, choose the threshold on validation data. If the output will be interpreted as a probability, check calibration separately instead of assuming the network did that for free.
\end{warningbox}

\begin{example}[Accurate Does Not Mean Calibrated]
Suppose a validation set contains twenty examples for which the network reports probabilities near \(0.9\). If only twelve of those twenty examples are actually positive, the classifier may still rank examples reasonably well and even achieve good thresholded accuracy, but the reported ``90\%'' confidence is overstated. Calibration asks whether predicted probabilities match observed frequencies, not whether the top class is often correct.
\end{example}
...
\begin{tabular}{L{2.2cm}L{1.9cm}L{2.0cm}L{2.7cm}}
\toprule
Object & One example & Batch of \(B\) examples & Meaning \\
\midrule
Input & \(x: 1\times 2\) & \(X: B\times 2\) & two input features per example \\
First-layer weights & \(W^{(1)}: 2\times 2\) & same & map two inputs to two hidden units \\
Hidden pre-activations & \(Z^{(1)}: 1\times 2\) & \(Z^{(1)}: B\times 2\) & weighted sums before ReLU \\
Hidden activations & \(H^{(1)}: 1\times 2\) & \(H^{(1)}: B\times 2\) & learned intermediate features after ReLU \\
Output weights & \(W^{(2)}: 2\times 1\) & same & combine hidden units into one score \\
Score / probability & \(s,\hat p: 1\times 1\) & \(s,\hat p: B\times 1\) & one logit and one probability per example \\
\bottomrule
\end{tabular}
\caption{Shape bookkeeping becomes much easier once one matrix convention is chosen and kept consistent. Here the batch is treated as a matrix of row examples.}
\label{tab:shape-bookkeeping}
\end{table}

\begin{practicalnote}
Students often find neural notation less confusing once every tensor is read as an answer to one question: how many examples are moving through the network at once, and how many features does each stage expose? Shape bookkeeping is not cosmetic. It is a fast way to catch mismatched layers, wrong output dimensions, and silent implementation errors.
\end{practicalnote}
...
Once the network outputs a score or probability, what tells it how to improve? The answer is the loss. The same worked network now stops being only a predictor and becomes an object that can be corrected.
...
A loss function measures how bad a model's prediction is on a single example. Training seeks parameter values that make the average loss over the training data small.
\end{definition}
...
\begin{mathlens}
For one example with logit \(s\), probability \(\hat{p}=\sigma(s)\), and label \(y\in\{0,1\}\), the loss is
\[
...
=\hat{p}-y.
\]
That is the reusable training signal for a sigmoid output layer: prediction minus truth.
\end{mathlens}
...
Later chapters will repeatedly use multiclass classification, so it is worth seeing the standard pattern now. A multiclass output layer usually produces one score per class, called a \emph{logit}. Softmax converts those logits into probabilities:
\[
...
Once a loss has been defined, training becomes an optimization problem: find parameter values that reduce that loss. Gradient-based optimization works by asking how the loss would change if each parameter were nudged slightly.
...
\subsection{Optimization Dynamics: Momentum, Adam, and AdamW}

The learning rate tells us how large a step to take, but it does not say how to shape the step when the loss surface is uneven. Modern update rules try to make optimization less brittle by using recent gradient information either to smooth motion or to scale updates per parameter.
...
\textbf{Adam} adds a second idea: keep running statistics of both the gradient and its squared magnitude. The first statistic tracks direction, and the second tracks scale. Parameters with consistently large gradients get smaller effective steps, while parameters with smaller gradients are not forced to share the same step size as the largest ones. In practice, this often makes early training easier and less sensitive to a single global step size.

\textbf{AdamW} keeps the adaptive step logic but separates weight decay from the gradient update itself. That separation matters because weight decay is a regularization choice, not a disguised learning-rate trick. Students should think of AdamW as saying: ``adapt the step, but still regularize the weights in the intended way.''
...
This distinction is foundational for debugging. Students who do not draw it clearly will add dropout when they should be fixing the output layer, or increase model capacity when they should be adding regularization. Both changes are reasonable in isolation; neither is helpful when the wrong diagnosis drives it. The first debugging reflex in this chapter should therefore be diagnostic, not architectural.

The cleanest diagnostic procedure is to first establish whether the model can overfit a very small dataset. Take a batch of ten to twenty training examples and train the model until training loss is very low. If the model cannot overfit even ten examples, the problem may be optimization, but it may also be label quality, excessive regularization, or insufficient capacity. If it can overfit ten examples but not generalize on the full dataset, the problem is more likely generalization. The separation is not always perfect, but the small-batch overfit test is the fastest way to decide which problem to solve first.

\begin{minimumtakeaway}
When a neural run looks bad, separate optimization failure from generalization failure before touching the architecture. If the network cannot fit a tiny batch, debug the training setup first; regularization and bigger models come later.
\end{minimumtakeaway}
...
Batch normalization is another way to make optimization less brittle. It normalizes intermediate activations using statistics computed from the current mini-batch, then restores flexibility with learned scale and shift parameters. The intuition is that if the hidden signals stay in a manageable range, the next layers see a more stable training problem.
...
\begin{practicalnote}
Batch normalization stabilizes the signal within a layer. A complementary technique called a \emph{residual connection} (or skip connection) helps the signal flow between layers. The idea is simple: instead of computing $h = f(x)$, the layer computes $h = f(x) + x$, adding the input directly to the output. If the transformation $f$ is unhelpful, the gradient can flow through the identity branch unchanged, so deep networks become much easier to train. Residual connections are a key design principle behind ResNets in vision and many transformer blocks in language.
\end{practicalnote}
...
\begin{thinkingtool}
\textbf{First debugging order.} When a first neural run goes wrong, reach for the levers in this order:
\begin{enumerate}[leftmargin=*]
\item verify that labels, output layer, and loss match the task;
\item test whether the model can overfit one tiny batch;
\item tune the learning rate and check normalization, initialization, and batch size;
\item only after optimization works, judge regularization, augmentation, or architecture changes.
\end{enumerate}
This ordering matters because regularization does not rescue a network that never learned, and more depth does not fix a mismatched loss-output pair.
\end{thinkingtool}
...
Now imagine the same network but with a hidden pre-activation $z=-0.5$. Then $h=\mathrm{ReLU}(-0.5)=0$, and the local derivative of ReLU is $0$ at that point in the usual teaching approximation. The backward chain therefore gives
\[
...
The Chapter 3 question returns in sharper form: are we reducing training loss in a way that helps on unseen data, or are we merely fitting noise more effectively?
...
Several common tools are used to control overfitting:
\begin{itemize}[leftmargin=*]
\item \textbf{weight decay} discourages large parameter values by penalizing them during optimization
\item \textbf{early stopping} halts training when validation performance stops improving
\item \textbf{dropout} randomly suppresses some activations during training so the network does not rely too heavily on any one pathway
\item \textbf{data augmentation} expands effective data variety by transforming inputs in label-preserving ways
\end{itemize}
...
\textbf{High label noise.} When labels contain substantial errors---from careless annotation or ambiguous task definitions---a higher-capacity model is more likely to memorize those errors than a lower-capacity one. Label leakage is a separate issue: it means target information has slipped into the features or split design, not that the labels themselves are noisy. Label quality and split logic should be checked before architectural decisions are made, because adding layers will not fix either problem.
...
By the time a neural result is reported, many quiet choices have already shaped it: the split, the preprocessing, the optimizer, the stopping rule, and the baseline it had to beat. If those choices stay implicit, the result is hard to judge. The training report below is a compact way to make them visible.
...
\begin{thinkingtool}
\textbf{Minimum credible neural training report.} The report becomes readable only when the data split, preprocessing, architecture, loss, optimizer, learning rate, batch size, stopping rule, randomness, and baseline comparison are all visible.
\end{thinkingtool}

\begin{example}[Filled Minimum Credible Neural Training Report]
Suppose the task is binary student-risk prediction from the first three weeks of course activity. A minimally credible report might say: chronological split by semester with $70\%$ train, $15\%$ validation, and $15\%$ test; train-only standardization of activity counts plus explicit missingness indicators; a two-layer MLP with input width matched to the engineered features, one hidden layer of modest size, and a sigmoid output; binary cross-entropy for training and area under the precision-recall curve plus escalation recall for evaluation; AdamW with a validated learning rate, fixed batch size, and early stopping on validation loss; three seeds rather than one lucky run; calibration checked on validation before probabilities are interpreted; and a logistic-regression baseline reported alongside the neural result. The report is not impressive because it is verbose. It is credible because it tells the reader what the network saw, how it was trained, and what simpler model it had to beat.
\end{example}
...
\small
\setlength{\tabcolsep}{5pt}
\begin{tabular}{p{2.7cm}p{7.3cm}}
\toprule
Item & One filled undergraduate example \\
\midrule
Task & predict whether a student should be flagged for human review after the first three course weeks \\
Split & chronological by semester: $70\%$ train, $15\%$ validation, $15\%$ test \\
Preprocessing & train-only standardization of activity counts; explicit missingness indicators for not-yet-observed fields \\
Architecture & two-layer MLP with one modest hidden layer and sigmoid output \\
Loss and metrics & binary cross-entropy for training; PR-AUC and escalation recall for evaluation \\
Decision rule & validation-set threshold chosen to keep reviewer workload feasible \\
Calibration & checked on validation before probability language is used in the report \\
Optimizer & AdamW \\
Learning rate / batch size & one validated learning rate; fixed batch size reported explicitly \\
Stopping rule & early stopping on validation loss \\
Randomness & three seeds with mean and spread reported \\
Baseline & logistic regression on the same engineered features \\
\bottomrule
\end{tabular}
\caption{A filled report is easier for students to imitate and easier for instructors to grade. The goal is not formality for its own sake; it is to make the neural claim auditable.}
\label{tab:filled-neural-training-report}
\end{table}
...
\caption{Representation learning is the real bridge to the next chapter. The model is not merely drawing a final boundary; it is building the space in which the final boundary becomes easier to draw.}
\label{fig:representation-hierarchy}
\end{figure}

Optimization is not only fitting a final boundary. It is teaching the model which internal coordinates make the task easier.

Once those learned coordinates become strong enough, they can outlive the original task. A model trained for one problem can supply a reusable space for later tasks, domains, and label budgets. That is the beginning of pretraining and transfer, and it is the idea the next chapter takes up.
...
\begin{mistakebox}
\item \textbf{Treating architecture as the whole story.} Data quality, preprocessing, metrics, and split design matter just as much.
\item \textbf{Ignoring simpler baselines.} If a neural network beats no baseline, the result is uninterpretable.
\item \textbf{Overreading training curves.} A single clean-looking training run proves little.
\item \textbf{Changing too many training choices at once.} Change a manageable number of factors at a time.
\item \textbf{Confusing confidence with understanding.} High confidence is not robustness, interpretability, or causal insight.
\item \textbf{Thinking bigger automatically means better.} Larger models are justified only when they produce better evidence.
\end{mistakebox}

\section{Looking Ahead}

Hidden features, loss, backpropagation, and regularization explain how neural learning works. The next question is when those learned spaces are reusable across tasks and how to tell whether transfer is really helping.

The minimum credible neural training report from this chapter becomes the starting document for Chapter 7's reuse and adaptation plan: once a network learns useful internal features, the next question is whether those features transfer honestly.

\begin{chaptersummary}
\item The central question here is when structure should be learned rather than engineered, and how optimization teaches a network to build it.
\item A neural network is best understood as a learned feature pipeline rather than as a bag of layers and tricks.
\item Nonlinearity matters because without it a deep network collapses back into one larger affine map, so depth alone does not create learned structure.
\item Losses provide the teaching signal for optimization, while evaluation metrics remain the tools for judgment.
\item Gradient descent is repeated local correction, and backpropagation is the mechanism that assigns credit or blame through the network.
\item Optimization choices and regularization are part of the experiment, which is why the minimum credible neural training report is part of the chapter's core discipline rather than an appendix.
\item Weight decay, early stopping, dropout, and augmentation all try to bias learning toward patterns that remain stable on new data.
\item The payoff of the chapter is representation learning: optimization is teaching the model which internal coordinates make the task easier.
\end{chaptersummary}
...
\begin{exercises}
\item Compute the forward pass of a one-hidden-layer network with input $x=[1,2]$, hidden unit $h=\mathrm{ReLU}(2x_1-x_2-1)$, and output score $s=1.5h-0.5$. Give the final score and the sigmoid probability.
\item Evaluate sigmoid, $\tanh$, and ReLU at $z=-2$, $z=0$, and $z=2$. What qualitative differences do you observe?
\item Suppose the true label is $1$ and two models predict probabilities $0.55$ and $0.95$. Compute the binary cross-entropy loss for both and explain why the difference matters even though both predictions are correct at a $0.5$ threshold.
\item A multiclass classifier outputs logits $(1.0, 0.0, -1.0)$. Compute the softmax probabilities approximately and state the cross-entropy loss if class 1 is the true class.
\item For the scalar model $\hat{y}=wx$, let $x=3$, $y=9$, and current $w=1$. Using squared error loss, compute the gradient with respect to $w$ and perform one gradient descent update with learning rate $0.05$.
\item Explain in words why backpropagation is a credit-assignment procedure rather than just a formula-manipulation trick.
\item A mini-batch contains \(32\) examples, each input has dimension \(16\), the hidden layer has \(8\) units, and the output layer has \(3\) logits. Using the row-batch convention from Table~\ref{tab:shape-bookkeeping}, give the shapes of \(X\), \(W^{(1)}\), \(Z^{(1)}\), \(H^{(1)}\), \(W^{(2)}\), and the logits.
\item A classifier is \(92\%\) accurate on validation data. Among the examples assigned probability near \(0.9\), only \(60\%\) are actually positive. What does this say about calibration, and what extra check should be reported before treating the outputs as probabilities?
\item A training run shows train/validation losses \((0.69,0.69)\), \((0.69,0.68)\), \((0.68,0.69)\), and \((0.68,0.69)\) over four epochs. Diagnose the main failure type and name the first two things you would inspect.
\item A binary classifier shows the following training and validation losses across six epochs: $(0.69,0.70)$, $(0.61,0.63)$, $(0.53,0.58)$, $(0.44,0.60)$, $(0.36,0.66)$, $(0.29,0.74)$. Diagnose the main failure pattern, and name one change you would try first.
\item Another run reaches very high training and validation performance almost immediately on a task where some features may contain post-outcome information. Explain why this pattern is suspicious, and state what should be audited before celebrating.
\item Using Table~\ref{tab:minimum-credible-training-report}, sketch a minimum credible training report for a small binary classifier and name the simpler baseline it must beat. If you are carrying one case through the book, use the same case as your structure diagnosis sheet and state which failure pattern justified moving to a neural model.
\item A project report says only ``two-layer MLP, Adam, 94\% accuracy.'' Name four missing items from the chapter's minimum credible training report that you would need before trusting the result.
\item Give one example of a task for which a neural network may be justified because representation learning matters, and one example of a task where a simpler model may still be preferable. State what evidence would decide between them.
\item A training run shows steadily falling training loss but worsening validation performance after epoch 12. Name the failure type, one sensible intervention, and whether the next move should be better regularization, a better baseline comparison, or a transfer-learning workflow from Chapter 7.
\item A run cannot overfit a batch of 12 examples. Using the chapter's debugging order, name the first three questions you would check before increasing model depth.
\end{exercises}

\begin{challengeexercises}
\item Consider a two-layer network with no activation between layers. Write the full expression for the output and show algebraically why it can be rewritten as one affine transformation.
\item In the worked backpropagation example, recompute the gradients if the hidden pre-activation had been $z=-0.5$ instead of $z=1$. What changes, and what does that say about ReLU units?
\item A model family with many parameters performs worse than logistic regression on a small text-classification dataset. Give three technically plausible explanations that do not reduce to ``neural networks are bad.''
\item Pick one public learning task of your choice. Propose a minimum neural training plan: output layer, loss, optimizer, batch size, validation strategy, and baseline.
\item Two neural runs differ in architecture, optimizer, batch size, and augmentation policy, and one of them scores higher by one percentage point on the validation set. Explain why this is not yet convincing evidence of a better model.
