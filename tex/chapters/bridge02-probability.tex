...
This bridge gives the probability and optimization vocabulary that later training objectives will assume. By the end, the reader should be able to distinguish clearly between a score, a probability, a decision rule, a pointwise loss, and a dataset-level training objective; read conditional probability and expectation in plain language before manipulating formulas; explain why logarithms, exponentials, sigmoid-style transformations, and cross-entropy appear naturally in ML; interpret empirical risk as the average training signal over a dataset rather than as an abstract formula to memorize; and read gradients and the chain rule as credit-assignment tools for reducing training loss. The functional view of predictors and risks, and the contrast between generative and discriminative models with priors and MAP as preview vocabulary, are second-pass material---useful for later chapters but not required before continuing.
...
\begin{practicalnote}
\textbf{Core path.} For a first pass, focus on scores, probabilities, decisions, losses, empirical risk, and gradients as training signals. The functional viewpoint, generative/discriminative contrast, priors, and MAP are orientation material for later chapters; they can be skimmed until those later chapters need them.
\end{practicalnote}
...
\begin{itemize}[leftmargin=*]
\item the benefit of a correct intervention: if it prevents serious academic harm, the benefit is large,
...
\]
The derivation is only an expected-cost comparison. If the posterior probability of the positive class is \(p\), predicting positive has expected mistake cost \((1-p)C_{FP}\), while predicting negative has expected mistake cost \(pC_{FN}\). Predict positive when
\[
(1-p)C_{FP} < pC_{FN}.
\]
Solving this inequality gives the threshold above. If false negatives are nine times more costly than false positives, the optimal threshold is \(1/(1+9)=0.1\), not \(0.5\). The default threshold of \(0.5\) therefore encodes an implicit equal-cost assumption that is often wrong in practice. Base-rate changes still matter, but they matter through the calibrated posterior probabilities the threshold is applied to.
...
= \frac{0.06}{0.24}=0.25.
\]
The keyword raised the probability from \(0.10\) to \(0.25\), but the action still depends on cost and capacity. If urgent misses are very costly, \(0.25\) may justify review. If review slots are scarce and the keyword is common, the same probability may be too low for automatic escalation.
\end{example}

\begin{practicalnote}
\textbf{Learning-signal card checkpoint.} At this point the card should already distinguish the model output from the decision rule. A probability such as \(0.25\) belongs in the model-output row; the outreach threshold belongs in the decision-rule row.
\end{practicalnote}
...
Why does this matter for AI practice? When a model output drives a threshold decision, the calibration of that output determines whether the threshold can be set reliably from a stated probability. A model that outputs $0.82$ for cases whose empirical event rate is much lower will, under a threshold-as-policy interpretation, flag many cases that a properly calibrated model would not flag. The team may then tighten the threshold empirically, but without understanding that the underlying model probabilities are systematically distorted, they cannot trust that the threshold will transfer to a new semester, a new patient cohort, or a new deployment context.
...
\begin{itemize}[leftmargin=*]
\item A discriminative model focuses all its capacity on the decision boundary. It is often more accurate for classification, but it does not model the data distribution directly, so it has no built-in density for outlier detection or sampling new data.
