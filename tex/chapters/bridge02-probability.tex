...
\begin{practicalnote}
\textbf{Learning-signal card checkpoint.} The card should already distinguish the model output from the decision rule. A probability such as \(0.25\) belongs in the model-output row; the outreach threshold belongs in the decision-rule row.
\end{practicalnote}
...
This section does not require graduate-level analysis. It asks for one conceptual shift: stop looking only at the coordinates of \(\theta\), and start looking at the whole predictor that training is selecting.
...
\begin{advancednote}
This section does \emph{not} need graduate functional analysis. The undergraduate level is enough: predictor as function, risk as a rule on predictors, norm as size or complexity measure, projection as fitting intuition. We do not need Hahn--Banach, compactness theorems, or abstract Banach spaces to make loss functions readable.
\end{advancednote}
