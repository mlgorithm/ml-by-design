...
\begin{advancednote}
\textbf{Why averaging is not enough: the correlated-tree variance bound.} The clean $\sigma^2/T$ formula above assumes the trees are mutually independent, which is rarely true: bootstrap samples overlap, and trees fit on similar data tend to make similar mistakes. Let the $T$ trees each have variance $\sigma^2$ and average pairwise correlation $\rho \in [0,1]$. A direct calculation gives
\[
\operatorname{Var}\!\left(\frac{1}{T}\sum_{t=1}^{T} \hat{y}_t\right) = \rho\,\sigma^2 + \frac{(1-\rho)\,\sigma^2}{T}.
\]

The operational consequence is sharp. As $T\to\infty$ the variance does \emph{not} vanish; it asymptotes to $\rho\sigma^2$. Adding more trees beyond a point cannot push the ensemble variance below this floor. The only way to drive variance down is to reduce $\rho$---to decorrelate the trees themselves. This is the formal reason random forests combine two sources of randomness rather than one: bootstrap resampling (bagging) alone leaves trees too correlated because they still see most of the same data and split on the same dominant features; the random feature subset at each split is what knocks $\rho$ further down. Each device makes the trees less similar, and each fraction of $\rho$ removed shows up directly in the variance bound.

For an interested reader, the derivation is one line: $\operatorname{Var}(\tfrac{1}{T}\sum_t \hat{y}_t) = \tfrac{1}{T^2}\bigl[\sum_t \operatorname{Var}(\hat{y}_t) + \sum_{i\neq j}\operatorname{Cov}(\hat{y}_i,\hat{y}_j)\bigr] = \tfrac{1}{T^2}\bigl[T\sigma^2 + T(T-1)\rho\sigma^2\bigr]$, which simplifies to the displayed expression.
\end{advancednote}
...
\section{Margin-Based Classification: Support Vector Machines (Extension)}
...
\begin{prereqbridge}
The math below uses the convention $y_i \in \{-1, +1\}$ instead of the $\{0, 1\}$ used in logistic regression. The reason is geometric: with $\pm 1$ labels, the product $y_i \cdot f(x_i)$ is positive when the prediction agrees with the label and negative when it disagrees, so ``margin'' (signed distance from the boundary) becomes a single number to maximize.
\end{prereqbridge}
...
\textbf{The dual and the kernel trick (Extension).} The primal SVM solves for the weight vector $w$ directly. A classical alternative is the \emph{dual} formulation. Introducing Lagrange multipliers $\alpha_i \ge 0$ for the margin constraints and eliminating $w$ and $b$ via the KKT optimality conditions transforms the hard-margin problem into
\[
\max_{\alpha \ge 0} \quad \sum_i \alpha_i - \tfrac{1}{2}\sum_{i,j} \alpha_i \alpha_j\, y_i y_j\, (x_i^\top x_j) \quad \text{subject to } \sum_i \alpha_i y_i = 0,
\]
with the soft-margin version adding box constraints $\alpha_i \le C$. The optimal weight is $w^\star = \sum_i \alpha_i^\star y_i x_i$, and only the points with $\alpha_i^\star > 0$ (the support vectors) contribute. Two things follow. First, the dual's complexity scales with the number of examples, not the dimension---useful when $d \gg n$. Second, the data appears only through inner products $x_i^\top x_j$, which opens the door to the kernel trick.

The \emph{kernel trick} replaces every inner product $x_i^\top x_j$ by $K(x_i, x_j) = \phi(x_i)^\top \phi(x_j)$ for some feature map $\phi$ into a (possibly very high-dimensional) space. The SVM then learns a linear separator in $\phi$-space, which corresponds to a nonlinear boundary in the original input space, \emph{without} ever computing $\phi(x)$ explicitly. The usual choices are the polynomial kernel $K(x, x') = (1 + x^\top x')^p$ and the RBF (Gaussian) kernel $K(x, x') = \exp(-\gamma \|x - x'\|_2^2)$. Mercer's theorem characterizes which symmetric functions $K$ are valid inner products in some feature space: $K$ must be positive semi-definite, meaning the Gram matrix $K_{ij} = K(x_i, x_j)$ is PSD for every finite sample. The same trick underlies kernel ridge regression, kernel PCA, and Gaussian processes. For this book the operational summary is enough: kernels let a linear method see nonlinear structure by changing what ``inner product'' means.
...
\section{Naive Bayes: Probability as Structure (Preview) (Extension)}
