...
\begin{learninggoals}
\item Diagnose what kind of structure a failed linear baseline is missing.
\item Match conditional thresholds, margins, class-conditional signal, local similarity, or unlabeled organization to suitable model families.
\item Explain why trees, SVMs, Naive Bayes, nearest neighbors, clustering, and dimensionality reduction answer different structural questions.
\item Compare model families by the structure they add rather than by generic strength.
\item Write a structural diagnostic that justifies the next modeling move.
\end{learninggoals}
...
\item \exercisetag{Concept}{Core}Classify the query student in Table~\ref{tab:knn-scaling-data} with $k=1$ and $k=3$ before and after scaling. Explain why the prediction changes.
...
\item \exercisetag{Calculation}{Intermediate}Run one k-means iteration by hand for the two-cluster example in Figure~\ref{fig:kmeans-iteration}. State the assignments and the recomputed centers.
