...
A support vector machine is a classifier that chooses a separating hyperplane with the largest margin between classes. The \emph{support vectors} are the training points closest to the boundary, so they are the examples that actually determine that margin. When perfect separation is not realistic, the \emph{soft-margin} version allows some violations and penalizes them instead of demanding zero mistakes.
\end{definition}
