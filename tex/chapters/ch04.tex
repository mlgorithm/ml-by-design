...
A failed linear baseline does not merely say that the task is harder than expected. More often, it says something specific about the kind of structure that is missing. Sometimes the missing structure is conditional and region-specific; sometimes the current feature space needs a margin-based separator; sometimes simple class-conditional assumptions give a transparent probabilistic baseline; sometimes the relevant structure is local and geometric; and sometimes the real task is not supervised prediction at all but exploratory organization. This chapter is about reading that failure correctly so that the next model family is chosen for a structural reason rather than out of habit.
...
But it is not the only answer. Sometimes the right story is that the representation already gives a plausible geometry and the task needs a wide-margin separator. Sometimes simple class-conditional assumptions give a useful probabilistic baseline. Sometimes similar examples should borrow judgment from one another. Sometimes labels are absent and the job is exploratory organization rather than supervised prediction.

The opening question of the chapter should therefore be stated brutally clearly:

\begin{center}
\emph{When the linear baseline fails, is the missing structure mainly conditional thresholds and interactions, margin structure, class-conditional probabilistic structure, local similarity, or unlabeled organization?}
\end{center}
...
Before changing model families, state what structural mismatch the baseline has revealed. If the task sounds like branching logic, think trees. If the representation already looks separable and you want a robust boundary, think SVM. If the task has many sparse features and a transparent class-conditional baseline is useful, think Naive Bayes. If prediction should come from nearby similar cases, think neighbors. If labels are absent and you are looking for organization or compression, think clustering or dimensionality reduction.
...
Margin structure in a defended feature space & Support vector machines & Prefer wide-margin separators when the representation is already plausible \\
Class-conditional signal with many features & Naive Bayes & Use simple likelihood assumptions as a transparent probabilistic baseline \\
...
\bottomrule
\end{tabular}
\caption{This chapter is one diagnostic with five answers. The point is not to memorize method names, but to connect model families to structural mismatch.}
\label{tab:structure-map}
\end{table}

Keep one tiny student-profile cloud in mind throughout the chapter. Students A and B have around 50{,}000 clicks and at most one late submission and look safe; students C and D have around 20{,}000 clicks and four or five late submissions and look risky. Trees read that cloud as threshold logic, SVMs read it as a margin problem when the feature geometry is plausible, Naive Bayes reads it through class-conditional feature likelihoods, k-NN reads it as distance, k-means reads it as grouping, and PCA reads it as a dominant direction of variation. The better question is therefore not ``Which stronger model should we try next?'' but ``What specific structure did the baseline miss?''
...
Moving beyond the linear baseline is not a choice to make at random. It is a diagnostic response to a specific structural deficit. When a linear model falls short, it usually does so in several identifiable ways, and each way points toward a different family of methods.
...
\[
\frac{\lambda}{2}\|w\|_2^2 + C\sum_i \max(0, 1-y_i f(x_i)).
\]
The first term prefers a wide margin, with \(\lambda\) controlling the regularization strength. The second term is the \emph{hinge loss}, weighted by \(C\): it is zero when an example is safely beyond the margin and grows linearly when that example falls inside the margin or on the wrong side of the boundary.
\end{mathlens}
