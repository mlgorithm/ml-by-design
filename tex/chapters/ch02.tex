...
Chapter 1 defined the problem. Chapter 2 asks what it would mean for a prediction to be good enough to support the decision that problem requires. That question is narrower than model choice and more important than it first appears. A model is useful only relative to the quantity used to judge it, the loss used to train it, the rule that turns its scores into actions, and the standard by which those scores are interpreted as confidence.

This chapter separates those pieces carefully. It distinguishes the number optimized during learning from the number reported after training, then shows how thresholds, rankings, and abstention rules turn scores into actions. It closes by asking whether those scores can be trusted as probabilities. By the end of the chapter, the reader should be able to write a short metric-and-action plan that matches the real decision rather than merely reporting a convenient score.

\begin{learninggoals}
\item Separate training losses from the metrics used to justify a model.
\item Choose metrics that match the decision, action capacity, and cost of mistakes.
\item Explain how thresholds, top-$k$ rules, ranking metrics, and abstention turn scores into actions.
\item Recognize when calibration matters because a score is being treated as confidence.
\item Write a metric-and-action worksheet for a framed learning problem.
\end{learninggoals}

\begin{problemsolvinglens}
This chapter turns framed tasks into judged behavior. The core move is to separate the penalty minimized during training, the behavior reported after training, the rule that turns scores into actions, and the confidence interpretation that makes that rule defensible.
\end{problemsolvinglens}

\begin{thinkingtool}
\textbf{Losses are for learning; metrics are for claims.} Training needs smooth, stable penalties to optimize. Reporting needs quantities that match the decision, the review budget, and the action rule that another human being would have to defend.
\end{thinkingtool}

\section{Opening Question: What Would Count as Good Here?}

Return to the broader course-support setting from Chapter 1. Suppose the same organization wants an early-warning score for an introductory programming course. The score estimates whether a student is at risk of the Chapter 1 proxy event: missing one of the first two major deadlines or failing to submit early core work. Staff can contact at most five students this week.
...
In some systems, the action is thresholded: act on every case above a cutoff. In others, the action is budgeted: act only on the highest-scoring $k$ cases. In still others, the system is allowed to abstain, meaning that uncertain cases are deferred to human review rather than forced into automatic action. In ranked retrieval tasks, later sections will also use reciprocal rank, which asks how soon the first relevant item appears.
...
%% ── Top row: main analysis pipeline ─────────────────────────────────────
\node[pipebox]  (loss)    at (-3.6, 0) {training loss};
\node[pipebox]  (score)   at (0,    0) {model score};
\node[pipebox]  (metric)  at (3.6,  0) {reported measure};
\node[pipebox]  (policy)  at (7.2,  0) {decision rule};
%% ── Bottom row: quality checks and output ────────────────────────────────
\node[checkbox] (cal)     at (3.6, -2.2) {confidence check};
\node[checkbox] (abstain) at (7.2, -2.2) {abstain or review};
\node[claimbox] (claim)   at (10.8,-2.2) {operational signal};
%% ── Row-1 horizontal arrows ──────────────────────────────────────────────
\draw[arrow] (loss)   -- (score);
\draw[arrow] (score)  -- (metric);
\draw[arrow] (metric) -- (policy);
%% ── Vertical drop arrows ─────────────────────────────────────────────────
\draw[arrow] (metric) -- (cal);
\draw[arrow] (policy) -- (abstain);
%% ── Abstain → claim: straight right ─────────────────────────────────────
\draw[arrow] (abstain) -- (claim);
%% ── Confidence check → operational signal: right along bottom ────────────
\draw[arrow] (cal.east) -- (abstain.west);
\end{tikzpicture}%
}
\caption{Chapter 2 is a judgment chain. A score does not become an operational signal until the chapter has answered how it will be judged, how it becomes a decision, and whether its probabilities can be trusted.}
...
An evaluation metric is a summary statistic used to judge model behavior on a dataset after training. It should express what success means for the real task rather than what is merely convenient for optimization. This book uses \emph{evaluation metric} and \emph{reported metric} interchangeably: both refer to the quantity shown to stakeholders after training, as distinguished from the training loss used during optimization.
\end{definition}
...
\begin{practicalnote}
This chapter focuses first on binary classification metrics. Later in the chapter, the same logic is extended to multiclass confusion matrices and to macro- versus micro-averaged summaries.
\end{practicalnote}

\begin{example}[Spam Filtering and Precision]
Spam filtering is naturally precision-sensitive. False positives are painful because legitimate messages get hidden or delayed. At large email-service scale, even a small false-positive rate can affect a large number of users \citep{gmail2023}. This is why spam systems are rarely judged by accuracy alone.
\end{example}
...
\]
The model is accurate only because the majority class dominates the dataset. This kind of skew, called class imbalance, makes accuracy unreliable as a default metric.
\end{example}
...
Once the book is clear about what behavior matters, it can ask which training signal is most useful for learning it.
...
\begin{mathlens}
The previous sentence can be stated sharply when the model is forced to predict one constant value for every example.
...
This subsection moves in three steps: why a smooth loss matters for optimization, what log loss computes, and what behavior log loss rewards.
...
Because zero-one loss changes abruptly, it is awkward to optimize directly. In practice, binary classifiers are usually trained with cross-entropy, or log loss, a smooth penalty that grows when the model assigns low probability to the true label. For a classifier in a yes-or-no task that predicts a probability $p$ for the positive class, the loss on one example is
\[
\ell(y,p) = -\bigl(y \log p + (1-y)\log(1-p)\bigr).
\]
This loss becomes small when the model assigns high probability to the correct class and large when it is confident and wrong. In words: log loss penalizes confident mistakes severely and rewards honest probabilities.

A deeper reason this loss matters is that it rewards honest probability forecasts rather than merely correct labels. The proper-scoring-rule literature makes that statement precise \citep{gneiting2007}.

\begin{theorem}[Log Loss Is Strictly Proper for a Bernoulli Event]
Let $Y \sim \mathrm{Bernoulli}(p^\star)$ with $p^\star \in (0,1)$, and let $q \in (0,1)$ be a forecast for $P(Y=1)$. The expected log loss
\[
...
Differentiate:
\[
L'(q)= -\frac{p^\star}{q} + \frac{1-p^\star}{1-q}
= \frac{q-p^\star}{q(1-q)}.
\]
The only stationary point is therefore $q=p^\star$. A second derivative check gives
\[
L''(q)=\frac{p^\star}{q^2}+\frac{1-p^\star}{(1-q)^2}>0,
\]
so $L(q)$ is strictly convex on $(0,1)$ and the stationary point is the unique minimizer.
\end{proof}

If $p^\star=0$ or $p^\star=1$, the minimizing forecast lies on the boundary at $q=0$ or $q=1$, respectively. The theorem above focuses on the interior case where the minimizer and derivative calculation both live inside $(0,1)$.

In plain language, this formalizes the chapter's main claim: when downstream decisions use probabilities, log loss rewards honest probability reports rather than merely correct labels.

\begin{practicalnote}
The Brier score shares the same larger idea: both are strictly proper scoring rules, so the forecaster does best in expectation by reporting the true probability rather than a distorted one \citep{gneiting2007}.
\end{practicalnote}
...
This closed-form rule is exact only in the simplified case where the score is a calibrated posterior probability and the decision problem is fully summarized by false-positive and false-negative costs. It is useful precisely because it makes those assumptions visible.
...
Some systems should not force every score into an automatic yes-or-no action. A model may abstain on borderline cases and route them to a human reviewer. This is especially sensible when the cost of a wrong automatic action is high or when the model's probabilities are not trustworthy enough for full automation. Chapter~\ref{ch:models-systems} returns to this systems view in detail, but the judgment lesson already belongs here: sometimes the best policy is ``predict, but do not act automatically.''
...
Threshold curves summarize possible operating points, but they still leave open a harder question: whether the score itself deserves to be read as numerical confidence. That is the point where calibration becomes part of the evaluation story rather than an optional extra.

\subsection{Multiclass Evaluation}

When there are more than two classes, the bookkeeping table gets larger but the judgment logic stays the same. A multiclass confusion matrix has one row and one column per class, and per-class precision and recall are still defined one class at a time by treating that class as ``positive'' and all other classes as ``not that class.'' The new question is how to combine those per-class summaries into one report.

\begin{example}[Macro Versus Micro in Course Message Routing]
Suppose a course-support router predicts one of three labels for each incoming message: \texttt{deadline}, \texttt{grading}, or \texttt{technical}. On a held-out set, the system produces the confusion matrix below, where rows are true labels and columns are predicted labels:
\[
\begin{array}{c|ccc}
 & \text{deadline} & \text{grading} & \text{technical} \\
\hline
\text{deadline} & 8 & 1 & 1 \\
\text{grading} & 2 & 24 & 4 \\
\text{technical} & 1 & 3 & 6
\end{array}
\]
For the \texttt{deadline} class, precision is \(8/(8+2+1)=8/11\approx 0.73\) because 11 messages were predicted as \texttt{deadline}, and recall is \(8/10=0.80\) because 10 deadline messages truly existed. For \texttt{grading}, precision is \(24/28\approx 0.86\) and recall is \(24/30=0.80\). For \texttt{technical}, precision is \(6/11\approx 0.55\) and recall is \(6/10=0.60\).
...
\]
The gap matters. Micro averaging says the system looks reasonably strong overall because the common \texttt{grading} class dominates the total. Macro averaging makes the weaker \texttt{technical} performance visible instead of letting it disappear inside the larger class.
\end{example}

\begin{practicalnote}
Use macro-averaged summaries when rare classes matter operationally and you want each class to count equally in the report. Use micro-averaged summaries when the deployed workload itself should determine the weighting. In practice, it is often best to report both whenever class imbalance is nontrivial.
\end{practicalnote}

\section{Confidence Quality: Calibration}

Thresholds and rankings are only part of the story. If the probabilities themselves are used for downstream decisions, the next judgment question is whether those probabilities mean what they appear to mean. A model can discriminate well enough to order cases correctly and still be unreliable as a source of numerical confidence.
...
\]
Lower is better, though a reliability diagram, which plots predicted probability against observed frequency across bins, still shows where the probability errors live.
...
\begin{example}[One Calibration Bin By Hand]
Suppose five cases all receive predicted probabilities between \(0.70\) and \(0.80\):
\[
0.71,\ 0.74,\ 0.75,\ 0.78,\ 0.79
\]
and their observed labels are \(1,1,0,1,0\). The mean predicted probability in the bin is \(0.754\), while the observed frequency is \(3/5=0.60\). In this bin the model is overconfident: it acts like roughly \(75\%\) of such cases should be positive, but only \(60\%\) were. A reliability diagram repeats this calculation across bins.
\end{example}
...
Suppose a student-support model flags $20$ students and, under the current semester conditions, $10$ of those flags are truly students who need intervention. Then
\[
\text{Precision} = \frac{10}{20} = 0.50.
\]
Now imagine the model is deployed in a calmer semester where truly at-risk students are less common, but the score behavior is otherwise similar. The model still flags $20$ students, yet only $5$ are truly positive:
\[
\text{Precision} = \frac{5}{20} = 0.25.
\]
Nothing in this toy example required the model to become worse at ranking. The environment changed because the base rate, also called prevalence, changed.
...
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{3.05cm}p{1.78cm}p{1.78cm}p{3.05cm}}
\toprule
Deployment setting & Students flagged & True positives among flagged & Precision \\
\midrule
Higher-prevalence semester & $20$ & $10$ & $10/20 = 0.50$ \\
Lower-prevalence semester & $20$ & $5$ & $5/20 = 0.25$ \\
\bottomrule
\end{tabular}
\caption{Precision moves when prevalence moves. A fixed threshold can therefore create very different review burden quality under different base rates.}
...
\subsection*{A Confidence Story That Runs Through the Book}

Calibration, abstention, and prediction intervals are three checkpoints on a single thread that continues well past this chapter. Each one asks a slightly different version of the same operational question: \emph{how confident should a downstream decision be in this particular score?} Calibration asks whether a stated probability behaves like an honest probability on average. Abstention asks whether the system should even act automatically on a given case. Prediction intervals and prediction sets ask whether the model can express its answer as a \emph{range of plausible outcomes} rather than a single number.

The distinction between a point prediction and an interval is pedagogically important. A regression model that reports ``expected delivery time is $42$ minutes'' gives operators no way to distinguish a confident estimate from a rough guess. The same model, reporting ``expected delivery time is $42$ minutes, with a $90\%$ interval from $31$ to $58$ minutes,'' invites a different class of decisions: whether to promise a time window, whether to request a human dispatcher's review, whether to queue a fallback courier. For classification, the analogous move is to output a prediction set---``the label is likely one of \{A, B\}, with about $90\%$ coverage''---rather than a single top-1 guess.

\begin{practicalnote}
A first-pass rule of thumb when designing an uncertainty-aware decision policy: decide in advance what the team will \emph{do differently} at low versus high confidence. If the downstream action is identical at $0.55$ and $0.95$, then calibration work and interval work cannot improve outcomes, and the team should invest elsewhere. If the actions differ---automatic approval versus human review, automatic dispatch versus manual scheduling---then confidence quality is on the critical path.
\end{practicalnote}

The same thread runs through later chapters. Chapter~\ref{ch:probabilistic-models} will show how Bayesian linear regression and Gaussian processes produce predictive distributions rather than point estimates, so that a regression answer naturally carries an interval. Chapter~\ref{ch:reliability} will return to uncertainty under the banner of reliability engineering (Section~\ref{sec:uq-reliability}): how to combine calibration, selective prediction, and conformal intervals into a deployment policy that is safe to trust. For now, the chapter-level takeaway is only this: when a point prediction is reported without any confidence signal, the decision policy that consumes it is implicitly assuming the number is trustworthy everywhere. Often it is not, and the operational cost of that assumption is invisible until a case near the boundary goes wrong.

\section{Validation, Cross-Validation, and Test Discipline}

By this point the chapter has separated metric from loss, score from action, and calibration from ranking quality. One more distinction is needed before any of those choices deserve trust: \emph{which data is allowed to choose them?} This is the bridge from Chapter 2's judgment logic to Chapter 3's evaluation discipline.

Thresholds, calibration methods, regularization strengths, stopping points, and even model families are not innocent details added after the score. They are part of model selection. If those choices are guided by the final test results, the test set stops being external evidence and becomes another development tool.

For this chapter, the core logic can be said plainly. The training data teaches the model parameters. The validation data chooses among serious alternatives: which threshold or ranking cutoff is usable, whether calibration is needed, and which tuned settings make the decision policy work best. The test data is then touched once, after those choices are frozen, to check whether the chosen policy still looks defensible on new cases.

\begin{example}[Threshold Choice Is Part of Model Selection]
Suppose a student-support team considers thresholds $0.40$, $0.55$, and $0.70$ on the validation set. Under the team's weekly review constraint, threshold $0.40$ catches more true positives but creates too many flags to review. Threshold $0.70$ keeps the queue small but misses too many students in need. Threshold $0.55$ gives the best recall while staying within review capacity, so it becomes the chosen policy. If the team then checks the test set, dislikes the result, and moves the threshold again, the test set has quietly become part of development. The number is still computed correctly, but it is no longer external evidence.
\end{example}

\subsection{Cross-Validation When Data Is Limited}

With abundant data, one protected validation split is often enough to choose among candidate settings. With limited data, a single split can be too noisy. Cross-validation repairs that weakness by rotating the validation role across several folds of the development data.

The main idea is simple. Partition the development data into several folds. Hold one fold out as validation, train on the remaining folds, record the decision-relevant metric, and rotate. At the end, average the fold-level validation results and choose the setting that behaves best on average.

The important point is conceptual rather than procedural. Cross-validation is not a replacement for external evidence. It is a stronger way to use the development data when one validation split would be unstable. If a final test set is available, it should still remain outside the cross-validation loop.

\begin{codeexample}[caption={Selection discipline when the decision policy is part of the model choice}]
...
for setting in candidate_settings:
    fold_values = []
    for train_fold, validation_fold in make_folds(development_data):
        model = fit_model(train_fold, setting)
        scores = model.predict_scores(validation_fold)
        value = evaluate_policy(scores, validation_fold.labels, setting)
...
In this skeleton, \texttt{setting} may include more than one kind of choice: a regularization strength, a tree depth, a threshold, a ranking cutoff, or a calibration rule. That is the deeper lesson. The decision policy is not outside the experiment. It is part of what is being selected.

\subsection{Hyperparameter Search Without Test Leakage}

A \emph{hyperparameter} is a setting chosen by the experimenter rather than learned directly by the basic fitting rule. Examples include the number of neighbors in k-nearest neighbors, the depth of a tree, the strength of regularization, the learning rate of an optimizer, or the threshold that turns scores into action. These choices are often necessary. The mistake is not searching over them. The mistake is letting the final test result guide the search.

Once this is stated clearly, the right workflow becomes straightforward. Search over alternatives on training and validation data, or inside cross-validation on the development set. Freeze one configuration. Then evaluate once on the test set. The test set should answer ``did this chosen system survive new data?'' rather than ``which of these many systems should we keep?''

\begin{practicalnote}
This chapter introduces the logic because metric choice, threshold choice, calibration, and selection protocol belong to one judgment chain. The next chapter makes the workflow fully procedural by showing how protected splits, leakage, and baseline discipline keep that chain honest.
\end{practicalnote}

\begin{thinkingtool}
\textbf{Selection protocol.} Before reporting a score, write down which data role chooses the model family, tuned settings, threshold, ranking cutoff, and calibration method. If the test set helped choose any of them, the reported score is no longer an external check.
\end{thinkingtool}

\section{When a Good Metric Still Hides the Wrong Behavior}

Some evaluation failures are not mathematical errors but mismatches between the reported metric and the decision that actually matters. The quickest way to see this is to look at a few recurring cases in which a respectable number conceals weak operational behavior.
...
By the end of Chapter 2, the reader should be able to write down not only which number to report, but how that number will become an action and which held-out role is allowed to choose the policy details. The point is to make metric choice, threshold policy, calibration expectations, and selection discipline explicit before the workflow quietly hardens around a vague score. This worksheet is the Chapter 2 counterpart to Chapter 1's framing memo: Chapter 1 states the problem clearly; Chapter 2 states how success will be judged and operationalized.
...
\begin{thinkingtool}
\textbf{Metric and action worksheet.} Before reporting a model output, write down the decision context, primary metric, supporting metrics, threshold or ranking policy, selection protocol, calibration need, and abstention rule. If the action rule or the held-out selection rule is still fuzzy, the score is not yet ready to defend.
\end{thinkingtool}

\begin{practicalnote}
Use the worksheet progressively. After the metric section, fill the primary and supporting metrics. After threshold and ranking sections, fill the action policy. After calibration, fill the confidence requirement. The final table should not be the first time the reader thinks about these fields.
\end{practicalnote}

\section{Looking Ahead}

This chapter made predictive judgment explicit: metrics, losses, thresholds, rankings, calibration, and abstention. Once the metric, action rule, and confidence interpretation are in place, the remaining question is whether the evidence for those choices can survive protected evaluation on new data.

\section*{Chapter Summary}

Chapter 2 defines what good predictive behavior means once a task has been framed. It separates the loss used to teach a model from the metrics used to justify it, then shows how thresholds, top-of-list policies, and abstention rules turn scores into operational decisions under real cost and capacity constraints. It also argues that calibration matters whenever a score is supposed to behave like a probability rather than merely as a ranking signal. The chapter's endpoint is a metric-and-action worksheet that makes the decision context, the primary and supporting metrics, the action rule, and the confidence requirements explicit before a result is reported.
...
\begin{exercises}
\item \exercisetag{Concept}{Core}For the course-support example, explain why a model with lower training loss could still be less useful than a model with better top-five ranking quality.
\item \exercisetag{Concept}{Core}Draw a tiny four-example binary classification problem in which two models have the same accuracy but different log loss. Explain what the probability assignments say about the models.
\item \exercisetag{Concept}{Core}Give a real-world example where mean absolute error would be more appropriate than mean squared error, and justify your answer.
\item \exercisetag{Proof}{Advanced}Using the theorem on best constant predictors, explain why squared error prefers the mean while absolute error prefers a median. Then give one example where that difference would matter in practice.
\item \exercisetag{Calculation}{Intermediate}Using Table~\ref{tab:threshold-example}, compute the confusion matrix at threshold $0.3$ and verify the precision, recall, and F1 values by hand.
\item \exercisetag{Implementation}{Intermediate}Reproduce the threshold sweep from the chapter in plain Python without external libraries. For each threshold, print the confusion counts, accuracy, precision, recall, and F1 score.
\item \exercisetag{Calculation}{Intermediate}A ranked list of six candidate passages has relevance labels $[0,1,1,0,1,0]$ from rank $1$ to rank $6$, and there are four relevant passages in the collection overall. Compute precision@3, recall@3, and reciprocal rank.
\item \exercisetag{Diagnosis}{Intermediate}Explain why the weather-forecasting example is a calibration question rather than a threshold question.
\item \exercisetag{Concept}{Intermediate}A team compares three regularization strengths and two thresholds for the same classifier. Describe a protected workflow for choosing among the six settings, and explain where cross-validation could help if the dataset is small.
\item \exercisetag{Design}{Intermediate}Use Table~\ref{tab:metric-action-worksheet} to sketch a one-page metric-and-action worksheet for either spam filtering, course support, or another application of your choice. If you are carrying forward a framing memo from Chapter 1, use the same case and state which parts of the original memo stay fixed while the decision rule becomes more explicit.
\end{exercises}

\begin{challengeexercises}
\item \exercisetag{Calculation}{Advanced}Construct a tiny binary classification example with six data points for which two different thresholds produce the same accuracy but very different precision and recall. Show the confusion matrix for both thresholds.
\item \exercisetag{Concept}{Advanced}Suppose a hospital has resources to review only the highest-risk 10\% of cases each day. Explain why ranking quality and calibration may matter more than raw accuracy in that setting.
\item \exercisetag{Design}{Advanced}Extend the course-support example in Table~\ref{tab:student-threshold-example}. Propose a rule for handling the six students above threshold $0.5$ when staff can contact only five of them, and justify the rule using the chapter's evaluation vocabulary.
\item \exercisetag{Concept}{Advanced}A model is well ranked overall but slightly miscalibrated near the review boundary. Explain when that is an acceptable limitation and when it becomes a deployment problem.
\item \exercisetag{Concept}{Advanced}A team tunes thresholds by checking the test set after each change and keeps the best final operating point. Explain precisely why the final precision and recall are no longer external evidence, even if the confusion counts were computed honestly.
