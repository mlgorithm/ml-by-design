...
Consider the word ``bank'' in two sentences: ``The bank approved the loan'' (financial institution) and ``The river reached the bank'' (geographic feature). A static embedding assigns one fixed vector to ``bank'' regardless of context, so both senses collapse into a single point. Any downstream model using this embedding must disentangle the senses from surrounding words---a burden that contextual representations remove by producing different vectors for the same token in different contexts.
\end{example}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Sampling with temperature 0.7}: ``the midterm essay resubmission.'' (samples from a slightly flattened distribution---more varied but still coherent).
