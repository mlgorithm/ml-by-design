...
\begin{advancednote}
The next three sections are orientation for modern LLM workflows. The core chapter goal remains the context-and-grounding card: what information the task needs, what evidence must be retrieved, when the system should answer, and when it should escalate. Instruction tuning, prompting, and RAG security are important extensions, but they should not replace the simpler context audit.
\end{advancednote}
...
\textbf{Zero-shot} prompting describes the categories and asks for a label. \textbf{Few-shot} prompting includes two or three labeled examples before the new message. A \textbf{structured-decomposition variant} can be paired with either one by asking the workflow to identify the main concern, check whether policy evidence is needed, and only then select the category.
If the zero-shot result is already reliable under realistic message variation, adding examples or decomposition steps may not improve it. If the model makes consistent errors on edge cases, few-shot examples that cover those cases may help. If the routing logic is genuinely multi-step---for example, distinguishing a visa question from a course question requires identifying a specific entity type first---a decomposition version of either prompt may reduce errors by making the required checks explicit. The scaffold should support auditability, not persuade the reader that fluent intermediate text is proof.
\end{example}
