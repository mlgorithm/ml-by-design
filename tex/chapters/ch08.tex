...
\begin{example}[BPE Merges on a Tiny Vocabulary]
Byte-Pair Encoding (BPE) builds a subword vocabulary by repeatedly merging the most frequent adjacent symbol pair in a training corpus. Take a tiny training corpus with four word types and the following observed frequencies: \texttt{low} appears 5 times, \texttt{lower} 2 times, \texttt{newest} 6 times, and \texttt{widest} 3 times. The initial vocabulary is the set of characters plus an end-of-word marker \texttt{</w>}, and each word is written out as a sequence of these atomic symbols:
\begin{center}
\texttt{l o w </w>} (5), \quad \texttt{l o w e r </w>} (2), \quad \texttt{n e w e s t </w>} (6), \quad \texttt{w i d e s t </w>} (3).
\end{center}

\emph{Step 1 --- count adjacent pair frequencies.} Sum across word frequencies. The pair \texttt{(e, s)} appears once in \texttt{newest} and once in \texttt{widest}, contributing $6 + 3 = 9$. The pair \texttt{(s, t)} contributes the same $9$. The pair \texttt{(l, o)} contributes $5 + 2 = 7$. The pair \texttt{(o, w)} contributes $7$. Suppose ties are broken by symbol order, so the first merge target is \texttt{(e, s)} with count $9$.

\emph{Step 2 --- merge.} Replace every adjacent \texttt{e\ s} in the corpus with the new symbol \texttt{es} and add \texttt{es} to the vocabulary. The corpus becomes
\begin{center}
\texttt{l o w </w>} (5), \quad \texttt{l o w e r </w>} (2), \quad \texttt{n e w es t </w>} (6), \quad \texttt{w i d es t </w>} (3).
\end{center}

\emph{Step 3 --- repeat.} Recount pairs on the updated corpus. The pair \texttt{(es, t)} now appears in \texttt{newest} and \texttt{widest}, contributing $6 + 3 = 9$, the new top pair. Merge it to produce \texttt{est}, giving
\begin{center}
\texttt{l o w </w>} (5), \quad \texttt{l o w e r </w>} (2), \quad \texttt{n e w est </w>} (6), \quad \texttt{w i d est </w>} (3).
\end{center}

The procedure continues for a chosen number of merges; the merge \emph{count} is the design knob. The pedagogical point is that BPE decides what counts as a token by training-corpus frequency. A larger target vocabulary means fewer merges, shorter token sequences, but more unknown-token risk on out-of-distribution text. A smaller vocabulary means more aggressive merges, longer sequences, and more shared subword structure. Perplexity (developed later in this chapter) is only comparable when the vocabulary is held fixed, because each subword vocabulary partitions probability mass over a different set of units.
\end{example}
...
\textbf{Attention in one formula (Extension; the convex-combination intuition above is enough for a first pass).} For a sequence whose elements have already been mapped to query, key, and value vectors via learned projections, scaled dot-product attention computes
\[
\text{Attention}(Q, K, V) = \mathrm{softmax}\!\left(\frac{Q K^\top}{\sqrt{d_k}}\right) V,
\]
where $Q, K, V$ stack the per-token queries, keys, and values as rows and $d_k$ is the per-head key dimension. The scaling factor $\sqrt{d_k}$ keeps the dot products from growing with dimension and pushing the softmax into saturated regions. Each row of the output is the contextualized representation of one token---a convex combination of all values, weighted by how much each key matched that token's query. Multi-head attention runs several such mixers in parallel; positional encoding restores order-sensitivity to a mechanism that is otherwise permutation-equivariant. Chapter~\ref{ch:representation-foundation-models} develops attention as a representation mechanism: the derivation of multi-head shapes, the pre/post-norm transformer block, sinusoidal positional encoding, and the convex-combination theorem all live there.
...
\begin{example}[Attention on a Two-Token Sentence, by Hand]
Take a two-token toy with two-dimensional projections. Suppose the model has produced
\[
Q = \begin{bmatrix} 1 & 0 \\ 0.3 & 0.95 \end{bmatrix},
\qquad
K = \begin{bmatrix} 0.9 & 0.1 \\ 0.2 & 0.98 \end{bmatrix},
\qquad
V = \begin{bmatrix} 1 & 0 \\ 0 & 1 \end{bmatrix}
\]
(rows are tokens; columns are coordinates). The raw scores are
\[
Q K^\top = \begin{bmatrix} (1)(0.9){+}(0)(0.1) & (1)(0.2){+}(0)(0.98) \\ (0.3)(0.9){+}(0.95)(0.1) & (0.3)(0.2){+}(0.95)(0.98) \end{bmatrix} = \begin{bmatrix} 0.90 & 0.20 \\ 0.365 & 0.991 \end{bmatrix}.
\]
With $d_k=2$, divide by $\sqrt{2}\approx 1.414$:
\[
\frac{Q K^\top}{\sqrt{d_k}} \approx \begin{bmatrix} 0.636 & 0.141 \\ 0.258 & 0.701 \end{bmatrix}.
\]
...
\[
\mathrm{out}_1 \approx 0.78\,v_1 + 0.22\,v_2 = (0.78, 0.22), \qquad \mathrm{out}_2 \approx 0.21\,v_1 + 0.79\,v_2 = (0.21, 0.79).
\]
Two observations: weights sum to $1$ and are nonneg\-ative, so each output sits in the convex hull of $\{v_1, v_2\}$ (Chapter~\ref{ch:representation-foundation-models}'s convex-combination theorem); and the two output rows are noticeably different because token $1$'s query is most aligned with key $1$ while token $2$'s query is most aligned with key $2$. The mixing weights carry genuine information---each token reads off a different value---rather than collapsing to the symmetric average that the all-orthogonal toy would force.
\end{example}

\begin{advancednote}
\textbf{Attention's cost: why context length is the bottleneck.} For a sequence of length $T$ and per-head embedding dimension $d$, computing the score matrix $S = Q K^\top$ costs $O(T^2 d)$ time and $O(T^2)$ memory. The subsequent row-wise softmax is another $O(T^2)$. The value combination $\mathrm{softmax}(S)\, V$ is a second $O(T^2 d)$ matrix product. Self-attention is therefore \emph{quadratic in sequence length}: doubling the context window quadruples both compute and activation memory, holding $d$ fixed.

The operational consequence is severe. A $32{,}000$-token document is roughly $1{,}000\times$ more expensive in attention compute than a $1{,}000$-token paragraph---not $32\times$ as the linear analogy would suggest. This single fact is the bottleneck behind every long-context engineering decision: KV-cache size and quantization; sliding-window attention (as in Longformer and the Mistral architecture); sparse attention patterns; low-rank approximations (Linformer); kernel-based linear-attention variants (Performer); IO-aware exact implementations (FlashAttention); and the recurrence-based state-space models (Mamba \citep{gu2023mamba}) that try to bypass the quadratic regime entirely.

Training-time and inference-time context lengths are different cost regimes. During autoregressive inference, a KV cache stores the keys and values for past tokens so that producing the next token does not recompute all pairwise scores; the per-token incremental cost falls to $O(T d)$. But the memory for the KV cache still grows linearly with $T$, and it is that growing cache---not the per-step compute---that ultimately bounds the maximum context a deployed model can hold.
\end{advancednote}
...
The three families of decoding above can be written precisely. Let $z \in \mathbb{R}^V$ be the model's logits over a vocabulary of size $V$, and let $p_i = \mathrm{softmax}(z/T)_i = \exp(z_i/T)/\sum_j \exp(z_j/T)$ be the token distribution at temperature $T$. The temperature $T$ reshapes the same distribution: $T \to 0$ peaks all the mass on the argmax, $T = 1$ recovers the model's native distribution, and $T \to \infty$ flattens it toward uniform.
...
def top_k_sample(z, k, T, rng):
    # Keep only the top-k logits, then sample with temperature.
...
def nucleus_sample(z, p_thresh, T, rng):
    # Top-p / nucleus: smallest set whose cumulative softmax >= p_thresh.
    p = softmax(z, T)
...
\section*{Extension: Prompt Injection and Retrieval Poisoning}
\addcontentsline{toc}{section}{Extension: Prompt Injection and Retrieval Poisoning}
\label{sec:rag-security}

This section pulls in adversarial-input vocabulary from Chapter~\ref{ch:reliability}'s security treatment. A first-course reading can stop at the Looking Ahead section and return here when designing a RAG deployment.
...
The next chapter keeps the sequence view but shifts to audio and time series, where the discipline becomes explicit: what information was actually available at the decision moment, and what would count as cheating through time? Chapter~\ref{ch:audio-time} extends the prediction-time audit introduced briefly here into a full chapter's worth of structural decisions.

A parallel sequel follows soon after. Chapter~\ref{ch:ranking-recommendation} reframes the ranking and retrieval machinery introduced in this chapter as a recommendation and exposure problem, showing how the same top-of-list metrics behave very differently once the system's own outputs shape the training data it will see next.
...
\item Language tasks are organized by what context they need preserved. Bag-of-words is a serious baseline when local order does not matter; n-grams repair short-range order; contextual embeddings are needed when meaning genuinely depends on the surrounding sentence.
\item Lexical baselines (TF-IDF, BM25) remain meaningful evaluation references, not historical curiosities. They expose the cases where a deep model's gain came from retrieval rather than language modeling.
\item Retrieval is part of the system, not a wrapper around the model. The retrieval step changes the claim the system is making; a strong generator on weak retrieval is a different system from a weak generator on strong retrieval.
\item Grounding is the discipline of tying generated text to retrieved evidence. A fluent answer without a grounded citation path is not yet a trustworthy answer, no matter how plausible it sounds.
\item Top-of-list retrieval metrics (precision@k, recall@k, reciprocal rank) measure different things and answer different operational questions. Picking the wrong one will make a system look ready when it is not.
\item Abstention and escalation are first-class language-system behaviors. A support assistant that refuses to answer a high-risk question is not failing; a fluent answer to the same question often is.
\item Hallucination is not one phenomenon. The chapter distinguished retrieval failure, grounding failure, knowledge cutoff, and confident fabrication---each requires a different diagnostic and a different fix.
\item Language is already about modeling. Choosing tokenization, context window, retrieval design, grounding policy, and abstention threshold is the modeling work; the architecture name on the box does not absolve any of those decisions.
\end{chaptersummary}
