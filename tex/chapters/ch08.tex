...
\item Use prompting strategies to choose the simplest inference-time intervention that produces reliable behavior.
...
This chapter turns context and grounding into the main design questions for NLP. After reading it, students should identify whether a task is mainly labeling, retrieval, or grounded response generation before choosing models. The NLP context-and-grounding card extends the earlier representation audits into evidence-aware language workflows.
\end{whychaptermatters}

The vision chapter treated invariance, shortcuts, and deployment realism as central problems. Language creates a parallel but different challenge. The same words can mean different things in different contexts, while the same idea can be expressed through very different surface forms. A language system therefore has to decide what may be collapsed away, what must be preserved, and when external \emph{grounding}---tying the model's output to retrievable, verifiable external evidence---is required before fluent output can be trusted. The chapter follows that route in order: first decide the task shape and the context that must survive, then decide what representation preserves it, then decide how grounding and behavior design change the claim, and finally decide what evaluation would make the claim defensible.
...
\begin{tabular}{L{2.55cm}L{3.2cm}L{4.0cm}}
\toprule
NLP regime & Primary job & What the system must get right \\
\midrule
Classification / labeling & Assign a label, span, or slot & Preserve the local or token-level structure that controls the label \\
Retrieval / ranking & Find the right document or passage & Surface the evidence that should support the later action or answer \\
Grounded generation / escalation & Produce a bounded answer or defer safely & Stay faithful to retrieved evidence and refuse or escalate when the evidence is weak \\
\bottomrule
\end{tabular}
\caption{A first NLP decision is not the model name but the regime: labeling, retrieval, or grounded response generation. Each regime implies a different output, evaluation style, and failure pattern.}
\label{tab:nlp-three-regimes}
\end{table}
...
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{L{2.42cm}L{3.27cm}L{3.32cm}}
\toprule
Stage or representation & Late-submission message & Visa-attendance message \\
\midrule
Bag-of-words routing & likely deadline / illness queue & may notice \texttt{visa} or \texttt{attendance}, but lexical overlap alone does not yet set the safety boundary \\
N-gram or local repair & preserves phrases such as \texttt{without penalty} and nearby negation if it appears & preserves phrases such as \texttt{affect my visa}, which local counts can blur \\
Contextual encoder & ties \texttt{sick} and \texttt{submit tonight} to a routine policy request & recognizes that attendance here belongs to a higher-stakes advising workflow rather than a late-work request \\
Retrieval target & late-work and illness-extension rules & international-student advising policy plus the human contact path \\
Behavior policy & grounded answer if evidence is current and complete & escalate or hand off even if a relevant page is found \\
\bottomrule
\end{tabular}
\caption{Two similar-looking student messages become easier to teach when they are followed through the same pipeline. The needed structure and the safe behavior can diverge long before final generation.}
\label{tab:two-messages-one-pipeline}
\end{table}
...
The chapter now shifts from representation and training objectives to system behavior. Once a language model produces next-token probabilities, the system still needs a behavior policy.

\subsection{Decoding Changes the Behavior}

The previous section described the decoder as a neural network component. Here, \emph{decoding} means something different: the inference-time strategy for selecting tokens from the model's probability distribution.
...
\begin{decisionbox}
\textbf{When not to generate.}
\begin{itemize}[leftmargin=*]
\item Do not generate when the needed evidence is missing, stale, or conflicting.
\item Do not generate when the case falls into a high-risk policy category that already requires escalation.
\item Do not generate when retrieval found only partial support and the omitted condition could change the action.
\item Do not generate when the system cannot expose the source behind the answer it is about to give.
\end{itemize}
\end{decisionbox}
...
\subsection{Worked End-to-End Grounded Workflow: One Question Through the Pipeline}

Keep one question fixed long enough to see the whole grounded workflow:
\begin{quote}
\texttt{I missed Lab 3 because I was sick. Can I submit tonight without penalty?}
\end{quote}
The point is not that every support system should answer this question automatically. The point is that one message can now be followed through task identification, retrieval, answer policy, and evaluation without resetting the reader's context at each stage.

\begin{table}[t]
\centering
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{2.0cm}p{4.7cm}p{3.22cm}}
\toprule
Stage & What happens to the same question & What must be checked \\
\midrule
Task identification & The system recognizes a grounded policy question about late submission rather than a free-form chat request & The workflow is in the right NLP regime: grounded answering, not generic generation \\
Chunking and indexing & Course policy pages are split so the illness-extension condition and deadline exception remain in the same retrievable chunk & The key condition is not lost before retrieval even starts \\
First-pass retrieval & Candidates include the late-work rule, the illness-extension form instructions, and perhaps a nearby but incomplete FAQ & The right evidence appears high enough to make grounding possible \\
Reranking & The system promotes the current illness-extension rule above stale or merely related passages & Relevance, freshness, and authority all still hold after reranking \\
Answer or escalate & If the retrieved rule is current and complete, the system may answer conservatively; if the evidence is partial or conflicting, it should defer & Generation is allowed only when the evidence contract is actually satisfied \\
Grounded response & A safe answer says the student must file the illness-extension form within forty-eight hours and cites the current policy & The answer preserves the condition and does not add unsupported reassurance \\
Evaluation target & Review checks retrieval success, faithfulness to the passage, and whether escalation would have been preferable under the risk level & The system is judged as a pipeline, not only by answer fluency \\
\bottomrule
\end{tabular}
\caption{One grounded question carried through the full workflow. The same message now ties together task regime, retrieval quality, answer policy, and evaluation target in one uninterrupted path.}
\label{tab:end-to-end-grounded-workflow}
\end{table}

The same walkthrough also clarifies diagnosis. If the system answers from an outdated FAQ, that is mainly a retrieval freshness failure. If it retrieves the correct illness-extension rule but answers ``Yes, illness means no penalty'' without the forty-eight-hour condition, that is a synthesis failure under correct evidence. If the message instead asks about visa attendance, the first failure is task and escalation choice rather than answer wording. Keeping one question fixed makes those differences easier to see.

\begin{decisionbox}
When a grounded answer looks wrong, diagnose the pipeline in order: did the system identify the right task, retrieve current and complete evidence, compose the answer faithfully, and only then choose direct answering instead of escalation?
\end{decisionbox}
...
In grounded NLP, fluent generation is never the whole story. If the retrieved evidence is wrong, stale, partial, or missing, the safest fix is often retrieval or escalation policy, not a more eloquent generator.
\end{minimumtakeaway}
...
Preference alignment & Signals that distinguish preferred from less-preferred responses (typically, human raters rank alternative model outputs and a reward signal derived from those rankings further adjusts the model) & How to favor helpful, honest, and bounded behavior over merely plausible continuations \\
\bottomrule
\end{tabular}
...
\item \textbf{Chain-of-thought variant}: either zero-shot or few-shot, but the input asks the model to first identify the main concern in the message and then select the category.
\end{itemize}
...
Return to the same late-submission message in Table~\ref{tab:end-to-end-grounded-workflow}. By this point the chapter has already decided what the system is trying to do, what evidence it must retrieve, and when it should answer or escalate. Evaluation in NLP depends sharply on task shape. The evidence must match the behavior the system is supposed to produce. This is where the chapter protects the claim made by the earlier choices about context, grounding, prompting, and escalation.
...
For language models, one common measure is perplexity (introduced in the Math Lens of Section~\ref{sec:language-modeling}):
\[
\mathrm{PP} = \exp\left(-\frac{1}{T}\sum_{t=1}^{T}\log P(w_t \mid w_1,\ldots,w_{t-1})\right).
\]
This is the same quantity defined earlier---the inverse geometric mean probability per token. Lower perplexity means the model assigns higher probability to the observed text. This can be useful, but it does not directly measure factual accuracy, grounding quality, fairness, or user usefulness.
...
\begin{example}[Same Passage, Different Evaluation Judgment]
Suppose one answer says, ``Yes, illness means you can submit tonight,'' while another says, ``Only if the illness-extension form is filed within forty-eight hours, according to the retrieved policy.'' A fluency-focused score might prefer the shorter first answer, but the grounded-answer rubric correctly prefers the second because it preserves the condition, exposes the source, and stays inside the evidence.
\end{example}
...
This chapter treated language as context-dependent sequence structure. The next chapter keeps the sequential view but moves to audio and time series, where the central question becomes more explicit still: what information was actually available at the decision moment, and what would count as cheating through time?

The NLP context-and-grounding card from this chapter becomes the starting document for Chapter 10's sequential design card: once language workflow is explicit, the next question is what information is legal at the decision moment.
...
\item A support assistant answers, ``The add/drop deadline is October 15,'' and cites a retrieved passage that says, ``The withdrawal deadline is October 15; the add/drop deadline is September 5.'' What is the main failure: retrieval, grounding, or generation? Explain the smallest pipeline change you would test first.
\item The same generator is run twice on the question ``Can I submit tonight without penalty?'' In run A, retrieval returns the current illness-extension rule. In run B, retrieval returns last semester's FAQ. Name the main retrieval-contract failure in run B and state the safest output policy the system should use.
\item Two prompts are given the same retrieved evidence. One asks for a short direct answer and the other asks for a cautious cited answer. The model answers much more boldly under the first prompt. Explain what this says about prompt sensitivity and what stability test you would run before deployment.
\item Two answers are written from the same retrieved passage. Answer A is fluent but omits an exception clause. Answer B is slightly less polished but cites the clause and recommends escalation for ambiguity. Using the grounded-answer rubric from the chapter, say which answer is better and why a single fluency score would miss the difference.
