...
The book has framed the learning problem and fixed the standard of evidence. The next question is modeling economy: once the task, metric, and evaluation protocol are clear, what is the simplest model that deserves a serious test? Often the right answer is a linear model---but only when the representation already exposes the structure that matters. This chapter treats linear models not as old formulas to memorize but as a disciplined test of representation: can a carefully chosen encoding plus one weighted sum already solve the task well enough to justify itself?

The central claim is simple. A strong representation and a controlled linear baseline often go further than beginners expect, and when they fail, the failure pattern is usually informative. The chapter moves from feature maps to the generic weighted-sum pipeline, then to linear regression, logistic regression, coefficient reading, and regularization. Its concrete deliverable is a first baseline design sheet stating what the model sees, what it predicts, how its coefficients may be read safely, how it will be regularized, and what specific failure would justify leaving linearity.
...
In ordinary least squares, let \(X\) be the design matrix augmented with an intercept column, so the affine model \(w^\top x+b\) can be written as a single matrix product. If \(\hat{y}=X\hat{\beta}\) is the fitted value vector and \(r=y-\hat{y}\) is the residual vector, then
\[
X^\top r = 0.
\]
...
On structured tabular data, much of the useful signal already lives in reasonably chosen features. A linear model can exploit that signal immediately without enormous datasets. On sparse text classification, the additive effect of informative words is often enough. In both cases, if a larger model wins, it should win by a clear and justified margin.

Linear baselines are also diagnostic. If a linear model performs well, the problem may not need extreme expressiveness. If it performs badly, the failure teaches us something. Maybe nonlinear interactions matter. Maybe spatial or sequential structure matters. Maybe the feature representation is too weak. In each case, the linear model gives a clean reference point for deciding what additional structure is needed.

\begin{example}[Baseline Logic on Public Tasks]
For a tabular task such as sports-shot prediction, a standardized linear or logistic baseline is often the right first move. For a text task such as news topic classification, bag-of-words or TF--IDF plus logistic regression is still a meaningful reference point. The baseline is useful even when it loses, because the way it fails tells us what structure may be missing.
\end{example}
