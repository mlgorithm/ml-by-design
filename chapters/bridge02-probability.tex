...
Bayes-optimal decisions formalize this idea. Under a simple binary setup with costs $C_{FP}$ (cost of a false positive) and $C_{FN}$ (cost of a false negative), the optimal threshold $\tau$ satisfies
\[
\tau = \frac{C_{FP}}{C_{FP} + C_{FN}}.
\]
If false negatives are nine times more costly than false positives, the optimal threshold is $0.1$, not $0.5$. The default threshold of $0.5$ encodes an implicit equal-cost assumption that is often wrong in practice.
...
The weight moved from \(1\) toward \(3\) (the minimum). The gradient pointed downhill in the negative direction; subtracting it moved \(w\) uphill in the negative-gradient direction, closing the gap to the optimum.

\begin{center}
\begin{tikzpicture}[font=\small]
  % Axes
  \draw[->] (-0.3, 0) -- (5.5, 0) node[right] {$w$};
  ...
  % Key x-axis marks
  \foreach \x/\lbl in {1/{$w{=}1$}, 2.6/{$w{=}2.6$}, 3/{$w{=}3$}} {
    \draw (\x, 0.05) -- (\x, -0.05);
    \node[below] at (\x, -0.05) {\lbl};
  }
  % Points on the parabola
  \filldraw[fill=white, draw=black] (1, 4) circle (2pt) node[above right] {$(1,\,4)$};
  \filldraw[fill=black]             (2.6, 0.16) circle (2pt) node[above right] {$(2.6,\,0.16)$};
  \filldraw[fill=black]             (3, 0) circle (2pt) node[above right] {$(3,\,0)$};
  % Arrow showing the step along the x-axis
  \draw[->, thick] (1, -0.35) -- (2.6, -0.35)
        node[midway, below] {step $= 1.6$};
\end{tikzpicture}
\end{center}
