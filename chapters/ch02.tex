...
The chapter moves in a deliberate order. First, we ask what behavior should be judged. Then we separate reported metrics from training losses, turn scores into actions through thresholds or rankings, and inspect whether probabilities are trustworthy enough to support action or abstention.

\section*{How to Read This Chapter}

Chapter 2 stays on one question: once a model produces scores, what behavior should count as good? Read the chapter in three passes. First separate metric from loss. Then separate score from action. Finally ask whether the probabilities are trustworthy enough for thresholds, rankings, or abstention-to-review.
...
This opening question determines the rest of the chapter. We first need to know what behavior matters. Only after that do training losses make sense.
...
\node[pipebox]  (metric)  at (3.4, 0) {reported measure};
\node[pipebox]  (policy)  at (6.8, 0) {decision rule};
\node[pipebox]  (loss)    at (10.2,0) {training loss};
%% ── Bottom row: quality checks and output ────────────────────────────────
\node[checkbox] (cal)     at (3.4,-2.0) {confidence check};
\node[checkbox] (abstain) at (6.8,-2.0) {abstain or review};
\node[claimbox] (claim)   at (10.2,-2.0) {operational signal};
...
%% ── Two arrows to claim top — enter at different x positions ─────────────
%%   loss  → arrives 0.3 right of center top  (x=10.5)
%%   policy → arrives 0.3 left  of center top  (x= 9.9)
\draw[arrow] (loss.south)  -- (10.5,-1.575);
\draw[arrow] (policy.east) to[out=0, in=270] (9.9,-1.575);
%% ── Abstain → claim: short horizontal ───────────────────────────────────
\draw[arrow] (abstain.east) -- (claim.west);
%% ── Cal → claim: route below the bottom row to avoid overlap ─────────────
\draw[arrow] (cal.south) |- (10.2,-2.82) -- (claim.south);
\end{tikzpicture}%
}
...
Once the reported behavior is clear, training losses make more sense. A learning algorithm needs a rule for preferring one prediction over another during optimization, meaning during the process of adjusting the model to reduce error on the training data. That role is played by a loss function.
...
\item The value of $c$ that minimizes $\frac{1}{n}\sum_{i=1}^{n}|y_i-c|$ is any sample median, meaning any middle observed value when the targets are sorted.
\end{enumerate}
\end{theorem}
...
\]
so the minimum occurs at $c=\bar{y}$. For absolute error, if $c$ sits to the left of a median, more than half of the sorted targets lie to its right, so moving $c$ slightly right decreases more distances than it increases. The symmetric argument applies when $c$ lies to the right of a median. That is why any median minimizes total absolute deviation.
\end{proof}
\end{mathlens}
...
%% ── Annotation ───────────────────────────────────────────────────────────
\node[align=center, text width=3.8cm, font=\footnotesize, gray!60!black]
    at (2.3,0.83) {both models classify all\\four examples correctly};
\end{tikzpicture}%
}
...
Many models produce scores or probabilities, not final actions. A decision still has to be made. That action may involve a threshold, a top-$k$ ranking, meaning ``take the highest $k$ scores,'' or an abstention rule, meaning a rule that withholds automatic action on uncertain cases and sends them to human review.
...
Thresholds and rankings are only part of the story. If the probabilities themselves are used for downstream decisions, the next judgment question is whether those probabilities mean what they appear to mean.
...
By this point the chapter has separated metric from loss, score from action, and calibration from ranking quality. One more distinction is needed before any of those choices deserve trust: \emph{which data is allowed to choose them?}
...
For this chapter, the core logic can be said plainly. The training data teaches the model parameters. The validation data chooses among serious alternatives: which metric summary matters most, which threshold or ranking cutoff is usable, whether calibration is needed, and which tuned settings make the decision policy work best. The test data is then touched once, after those choices are frozen, to check whether the chosen policy still looks defensible on new cases.
...
Three cases where a standard metric looks good while the real situation is not.
...
\paragraph{Case two: good ROC-AUC with poor precision at the deployment budget.} Suppose a classifier achieves ROC-AUC of $0.88$ on a test set. That looks like a well-discriminating model. Now suppose the operations team can review at most $30$ flagged cases per week, which corresponds to a threshold that captures $80\%$ of true positives. At that threshold, the model flags $120$ cases per week to reach $30$ after filtering, but that means $75\%$ of the flags are false positives. Staff spend most of their time on cases that do not need intervention. ROC-AUC summarizes performance across all possible thresholds and says nothing about which threshold is actually usable under the team's capacity constraint. A good ROC curve does not guarantee operational usefulness at any specific threshold.
...
This chapter made predictive judgment explicit: metrics, losses, thresholds, rankings, calibration, and abstention. The next chapter asks a different question: how do we protect those judgments from overfitting, leakage, weak baselines, and accidental test-set reuse?
