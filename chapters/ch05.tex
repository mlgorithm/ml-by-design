...
The previous chapter broadened our idea of structure by introducing branching rules, local geometry, and unlabeled organization. Neural networks broaden it again, but in a different way. Instead of depending so heavily on manually chosen splits, distances, or handcrafted interactions, neural models learn internal representations directly through optimization. Standard references such as \citet{lecun2015} and \citet{goodfellow2016} treat this layered view as the central idea of modern deep learning.

The chapter should therefore answer one question clearly:
...
This chapter addresses a different modeling dilemma: when hand-engineered structure stops being enough, how can a model learn the intermediate features it needs? Neural networks matter here not as a catalog of architectures, but as a mechanism for learned representation through optimization.
\end{problemsolvinglens}

By the end of the chapter, the reader should be able to produce a minimum credible neural training report before making a serious model claim.

\begin{thinkingtool}
\textbf{Artifact preview: minimum credible neural training report.} Keep six reporting commitments visible while reading: the baseline to beat, the input representation and output type, the loss, the optimization setup, the stopping rule, and the evidence that separates optimization failure from generalization failure.
\end{thinkingtool}
...
A hidden layer is not extra decoration. It is a learned representation stage.
...
\]
This is still just another affine transformation of $x$. No matter how many such layers are stacked, without nonlinearity the whole network is equivalent to one larger linear model.
...
\end{scope}
%% Panel 2: tanh — shifted by 6.5cm (panel width ≈ 5.1 units × 1.2cm = 6.1cm, gap 0.4cm)
...
\begin{tikzpicture}[x=1cm, y=1cm, font=\small]
%% Input nodes
\node[draw=c6netBlue, fill=c6netBlue!12, circle, minimum size=0.9cm, thick]
  (x1) at (0, 1.8) {$x_1$};
\node[draw=c6netBlue, fill=c6netBlue!12, circle, minimum size=0.9cm, thick]
  (x2) at (0,-0.4) {$x_2$};
%% Hidden nodes
\node[draw=c6netGreen, fill=c6netGreen!12, circle, minimum size=0.9cm, thick]
  (h1) at (4.0, 2.4) {$h_1$};
\node[draw=c6netGreen, fill=c6netGreen!12, circle, minimum size=0.9cm, thick]
  (h2) at (4.0,-1.0) {$h_2$};
%% Output node
\node[draw=c6netBlue, fill=c6netBlue!12, circle, minimum size=0.9cm, thick]
  (out) at (8.0, 0.7) {$s$};
%% Edges with labels placed away from crossings:
%% x1->h1 (short, near top): label near x1 end
\draw[-Latex, thick, black!60] (x1) -- node[above, pos=0.25, font=\footnotesize] {$1.5$} (h1);
%% x2->h1 (crossing): label near h1 end to separate from x1->h2 label
\draw[-Latex, thick, black!60] (x2) -- node[above, pos=0.80, font=\footnotesize] {$0.5$} (h1);
%% x1->h2 (crossing): label near x1 end to separate from x2->h1 label
\draw[-Latex, thick, black!60] (x1) -- node[below, pos=0.20, font=\footnotesize] {$0.5$} (h2);
%% x2->h2 (short, near bottom): label near x2 end
\draw[-Latex, thick, black!60] (x2) -- node[below, pos=0.25, font=\footnotesize] {$1.5$} (h2);
%% h1->out and h2->out
\draw[-Latex, thick, black!60] (h1) -- node[above, pos=0.4,  font=\footnotesize] {$1.2$} (out);
\draw[-Latex, thick, black!60] (h2) -- node[below, pos=0.4,  font=\footnotesize] {$0.8$} (out);
%% Annotations outside the network
\node[c6netGreen, font=\footnotesize, align=center] at (4.0, 3.6) {ReLU\\hidden units};
\node[black!55, font=\footnotesize] at (9.4, 0.7) {$\hat{p}=\sigma(s)$};
\end{tikzpicture}%
}
\caption{The hand-worked network as a diagram. Drawing the units makes the later forward and backward calculations much easier to track.}
...
# Input: attendance fraction and missed-assignment fraction
x1 = 0.92   # attendance rate (high is good)
x2 = 0.10   # missed-assignment fraction (low is good)

# Hidden layer weights and biases (hardcoded for illustration)
w11, w12, b1 = 1.4, -0.8, -0.5   # weights into hidden unit 1
w21, w22, b2 = 0.6,  1.2, -0.4   # weights into hidden unit 2
...
v1, v2, b_out = 1.1, 0.9, -0.3   # output-layer weights and bias
...
import math
p = 1.0 / (1.0 + math.exp(-s))   # sigmoid: P(at-risk = 1)
...
\]
One hidden unit is now shut off entirely. This will matter again when we discuss backpropagation.

\section{Loss as the Teaching Signal}

Once the network outputs a score or probability, what tells it how to improve? The answer is the loss.
...
\midrule
Class 1 & $1.4$ & $0.64$ & loss contribution $-\log(0.64)\approx 0.45$ \\
Class 2 & $0.5$ & $0.26$ & larger loss if true \\
Class 3 & $-0.7$ & $0.10$ & much larger loss if true \\
\bottomrule
\end{tabular}
...
\foreach \r in {0.8,1.4,2.0,2.6}
  \draw[gray!55] (0,0) ellipse ({1.5*\r} and {\r});
\draw[-Latex, thick, blue] (-2.4,1.6) -- (-1.5,1.0);
\draw[-Latex, thick, blue] (-1.5,1.0) -- (-0.8,0.5);
\draw[-Latex, thick, blue] (-0.8,0.5) -- (-0.25,0.18);
\filldraw (0,0) circle (2pt);
\node at (0.4,-0.5) {minimum};
\node at (-2.0,2.1) {follow the local slope};
\end{tikzpicture}%
}
...
\centering
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[font=\small, box/.style={draw, rounded corners, align=center, minimum width=2.2cm, minimum height=0.8cm}]
\node[box] (data) at (0,0) {load batch};
\node[box] (forward) at (3.0,0) {forward pass};
\node[box] (loss) at (6.0,0) {compute loss};
\node[box] (backward) at (9.0,0) {backward pass};
\node[box] (update) at (12.0,0) {update weights};
\node[box] (validate) at (6.0,-1.8) {validation check};
\draw[-Latex, thick] (data) -- (forward);
\draw[-Latex, thick] (forward) -- (loss);
\draw[-Latex, thick] (loss) -- (backward);
\draw[-Latex, thick] (backward) -- (update);
\draw[-Latex, thick] (update.south) |- (validate.east);
\draw[-Latex, thick] (validate.west) -| (data.south);
\end{tikzpicture}%
}
\caption{The training loop is short, but each stage has a distinct role. Students should be able to explain what changes during the forward pass, backward pass, and update step.}
...
\centering
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[x=1.2cm,y=1.4cm,font=\small]
\draw[->] (-3.2,0) -- (3.2,0) node[right] {parameter value};
\draw[->] (0,0) -- (0,4.2) node[above] {loss};
\draw[thick,domain=-2.8:2.8,samples=80] plot (\x,{0.35*\x*\x + 0.3});
\filldraw (-2.2,2.0) circle (1.8pt);
\filldraw (-1.6,1.2) circle (1.8pt);
\filldraw (-1.1,0.72) circle (1.8pt);
\draw[-Latex, blue, thick] (-2.2,2.0) -- (-1.6,1.2);
\draw[-Latex, blue, thick] (-1.6,1.2) -- (-1.1,0.72);
\filldraw (2.2,2.0) circle (1.8pt);
\filldraw (0.2,0.31) circle (1.8pt);
\draw[-Latex, green!60!black, thick] (2.2,2.0) -- (0.2,0.31);
\filldraw (-2.2,2.0) circle (1.8pt);
\filldraw (1.4,0.99) circle (1.8pt);
\filldraw (-1.8,1.43) circle (1.8pt);
\draw[-Latex, red, thick] (-2.2,2.0) -- (1.4,0.99);
\draw[-Latex, red, thick] (1.4,0.99) -- (-1.8,1.43);
\node[blue] at (-2.2,3.5) {too small};
\node[green!60!black] at (1.7,3.4) {good};
\node[red] at (0.8,2.6) {too large};
\end{tikzpicture}%
}
...
# y: true label (1 = student is at risk)
y = 1
...
\begin{tikzpicture}[font=\small,
  fwdarrow/.style={-Latex, thick},
  bwdarrow/.style={-Latex, thick, dashed},
  box/.style={draw, rounded corners, align=center, minimum width=2.2cm, minimum height=0.9cm, fill=black!4}]
% Boxes (left to right: input weight, hidden unit, output unit, loss)
\node[box] (w)   at (0.0, 0) {input\\weight $w_{11}$};
\node[box] (h)   at (3.2, 0) {hidden\\unit $h_1$};
\node[box] (out) at (6.4, 0) {output\\unit $s$};
\node[box] (L)   at (9.6, 0) {loss\\$\mathcal{L}$};
% Forward arrows (left to right)
\draw[fwdarrow] (w)   -- node[above] {\tiny forward} (h);
\draw[fwdarrow] (h)   -- node[above] {\tiny forward} (out);
\draw[fwdarrow] (out) -- node[above] {\tiny forward} (L);
% Backward gradient arrows (right to left, below the nodes)
\draw[bwdarrow] (L.south)   to[bend right=30] node[below] {\tiny $\frac{d\mathcal{L}}{ds}=\hat{p}-y$}     (out.south);
\draw[bwdarrow] (out.south) to[bend right=30] node[below] {\tiny $\frac{d\mathcal{L}}{dh_1}$}             (h.south);
\draw[bwdarrow] (h.south)   to[bend right=30] node[below] {\tiny $\frac{d\mathcal{L}}{dw_{11}}$}          (w.south);
\end{tikzpicture}%
}
...
The best way to understand regularization in this chapter is not as a list of tricks. It is as a bias toward less brittle learned representations.
...
Real neural training involves more than equations on a board. Several practical choices strongly affect outcomes. By the end of this chapter, a reader should be able to produce a minimum credible training report before making a serious neural-network claim. This is the document that keeps flexible neural models tied back to baselines and evidence instead of hype.
...
Decision rule & the threshold, ranking rule, or calibration step used to turn scores into actions \\
...
%% Three rows: Vision / Language / Audio
%% Box centres at x = 1.5, 5.2, 8.9  (gap 3.7 cm >> min-width 2.8 cm + text overflow)
%% Rows separated by 2.6 cm so titles never touch the boxes above

%% ---- Vision (top row) ----
\node[font=\small\bfseries, black!70] at (5.2, 1.9) {Vision};
\node[rbox] (v1) at (1.5, 0.7) {pixels};
\node[rbox] (v2) at (5.2, 0.7) {edges / textures};
\node[rbox] (v3) at (8.9, 0.7) {parts / object cues};
\draw[arr] (v1) -- (v2);
\draw[arr] (v2) -- (v3);

%% ---- Language (middle row) ----
\node[font=\small\bfseries, black!70] at (5.2,-1.5) {Language};
\node[rbox] (l1) at (1.5,-2.7) {tokens};
\node[rbox] (l2) at (5.2,-2.7) {local phrase cues};
\node[rbox] (l3) at (8.9,-2.7) {contextual meaning};
\draw[arr] (l1) -- (l2);
\draw[arr] (l2) -- (l3);

%% ---- Audio (bottom row) ----
\node[font=\small\bfseries, black!70] at (5.2,-4.9) {Audio};
\node[rbox] (a1) at (1.5,-6.1) {waveform / spectrum};
\node[rbox] (a2) at (5.2,-6.1) {local frequency cues};
\node[rbox] (a3) at (8.9,-6.1) {rhythm / context};
\draw[arr] (a1) -- (a2);
\draw[arr] (a2) -- (a3);
\end{tikzpicture}%
}
...
This is also the bridge to the next chapter. Once representation learning becomes strong enough, we can pretrain models on broad data and reuse those learned spaces across many downstream tasks. That is the foundation-model story in one sentence.
...
This chapter explained how neural learning works: hidden features, loss, backpropagation, and regularization. The next chapter asks when those learned spaces are reusable across tasks and how to tell whether transfer is really helping.

\begin{chaptersummary}
\item This chapter answers one question: when should structure be learned rather than engineered, and how does optimization teach a network to build it?
